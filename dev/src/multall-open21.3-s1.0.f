...
C   OF THE  PARAMETER STATEMENTS IN "commall-open-21.3"                C
C                                                                      C
...
C
      ...
      GA = 1.4
      READ(5,*) DUMMY_INPUT
      READ(5,*,ERR=7000,END=7000) CP,GA,IFGAS
      READ(5,*) DUMMY_INPUT
 7000 CONTINUE
C
      WRITE(6,*) ' IFGAS = ', IFGAS
      IF(IFGAS.EQ.0) THEN
      RGAS = CP*(GA-1.0)/GA 
      WRITE(6,*) ' USING PERFECT GAS PROPERTIES, IFGAS = 0 '
...
C
      IF(CP.LT.0.0. OR .IFGAS.EQ.1) THEN
C'
      CP1  = 1272.5
      CP2  = 0.2125
      CP3  = 0.000015625
 ...
 7001 CONTINUE
C
      WRITE(6,*) ' GAS WITH CP VARYING WITH TEMPERTURE CHOSEN.'
      WRITE(6,*) ' IDEAL GAS PROPERTIES READ IN AS: '
...
C
      WRITE(6,*)
      WRITE(6,*) ' GAS PROPERTIES INPUT FROM A LOOK UP TABLE. '
      WRITE(6,*)
      CALL READ_TABLE
C
      END IF
...
C 
      ITIMST = 3
      READ(5,*,ERR=7002,END=7002) ITIMST
 7002 CONTINUE
C
...
C
      ...
      IF_RESTART  =  0
      IF_INV      =  0
      READ(5,*,ERR=7017) IF_RESTART, IF_INV 
      READ(5,*) DUMMY_INPUT 
 7017 CONTINUE
C
      IF(IF_INV.EQ.1) WRITE(6,*)     ' INVERSE DESIGN MODE CHOSEN.'
      WRITE(6,*)
      IF(IF_RESTART.EQ.1) WRITE(6,*) 
     &               ' OPTION TO START FROM A RESTART FILE CHOSEN.'
      WRITE(6,*)
C
C**************************************************************************
...
C     INPUT THE INLET ABSOLUTE STAGNATION TEMPERATURE VARIATION WITH SPAN.
      READ(5,*)       DUMMY_INPUT
      IF(IFGAS.EQ.0.OR.IFGAS.EQ.1) THEN
      ...
      END IF
C     IF USING STEAM,INPUT THE INLET ABSOLUTE STAGNATION ENTHALPY
C      VARIATION WITH SPAN.
      IF(IFGAS.EQ.3) THEN
      READ(5,*)      (HO_INN(K),K=1,KIN)
      WRITE(6,*)    ' INLET STAGNATION ENTHALPY VARIATION WITH SPAN.'
      WRITE(6,1730)  (HO_INN(K),K=1,KIN)
      END IF
C
...
C
C  SET THE INLET STAGNATION DENSITY
C
      IF(IFGAS.EQ.0.OR.IFGAS.EQ.1) THEN
      DO 598 K = 1,KIN
            ROSTAG_IN(K) = PO1(K)/(RGAS*TO1(K) )
            WRITE(6,*) 'KIN, STAGNATION DENSITY AT INLET =',
     &                  K, ROSTAG_IN(K)
  598 CONTINUE
      END IF
C
C   IF STEAM, ITERATE TO FIND THE STAGNATION DENSITY AND TEMPERATURE AT INLET.
C
      IF(IFGAS.EQ.3) THEN
C
           PHRATIO = 0.1
      DO 595 K=1,KIN
           HO_INN(K) = 1000.0*HO_INN(K)
           RONOW     = PO1(K)/(PHRATIO*HO_INN(K))
C
      DO 596 NITS = 1,10
            UNOW  = HO_INN(K) - PO1(K)/RONOW
C
            CALL TABSEARCH(RONOW,UNOW,
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
C
            HNOW    = UNOW + PNOW/RONOW
            PHRATIO = PNOW/(RONOW*HNOW)
            RONOW   = PO1(K)/(PHRATIO*HO_INN(K))
  596 CONTINUE
C
           WRITE(6,*)
           WRITE(6,*) 'KIN, STAGNATION DENSITY AT INLET =', K, RONOW
C
           ROSTAG_IN(K) = RONOW
           TO1(K)       = TNOW
C
...
C
      ...
      RO_MIN  = AMIN1(RO(IMID,1,KMID),RO(IMID,JM,KMID))
      ROLIM  = 0.5*RO_MIN
      RIJKM  = 2./(IM*JM*KM)
C
      DO 4001 K=1,KM
...
C     MINIMISE ROUNDING ERRORS.
      ...
      WRITE(6,*) ' LIMIT ON LOW DENSITY =      ', ROLIM
C
C****************************************************************************** 
C****************************************************************************** 
...
C*************************************************************************
C*************************************************************************
C     INPUT THE DATA FOR AN INVERSE MODE CALCULATION ON THE FIRST TIME STEP
C
      OPEN(UNIT=11,FILE='inverse.in')
C
      WRITE(6,*)
      WRITE(6,*) ' INPUTTING DATA FOR AN INVERSE CALCULATION.'
      WRITE(6,*)
C
      J_LEDGE  = JLE(1) 
      J_TEDGE  = JTE(1)
C
      READ(11,*) IFINV_I1, IFINV_IM
      WRITE(6,*) IFINV_I1, IFINV_IM
C
C     SET THE INPUT SPECIFIED PRESSURE ON THE I = 1 SURFACE
      IF(IFINV_I1.GT.0) THEN
      READ(11,*) NIN
      WRITE(6,*) ' NIN', NIN
      DO 4901  N = 1,NIN
          READ(11,*) X_IN(N),P_IN(N)
          WRITE(6,*) ' XIN, PIN ', X_IN(N),P_IN(N)
 4901 CONTINUE
C
      PAVG_I1 = 0.0
      DO 4902 J = J_LEDGE,J_TEDGE
          ARG =( (X(J,1)-X(J_LEDGE,1))/(X(J_TEDGE,1)-X(J_LEDGE,1)))
          CALL INTP(NIN,X_IN,P_IN,ARG,PSPEC_I1(J))
          WRITE(6,*) ' J,PSPEC_I1 ', J, PSPEC_I1(J)
          PAVG_I1 = PAVG_I1 + PSPEC_I1(J)
 4902 CONTINUE
      END IF
C
C     SET THE INPUT SPECIFIED PRESSURE ON THE I = IM SURFACE
      IF(IFINV_IM.GT.0) THEN
      READ(11,*) NIN
      WRITE(6,*) ' NIN', NIN
      DO 4903  N = 1,NIN
          READ(11,*) X_IN(N),P_IN(N)
...
C
      PAVG_IM = 0.0
      DO 4904 J = J_LEDGE,J_TEDGE
          ARG =( (X(J,1)-X(J_LEDGE,1))/(X(J_TEDGE,1)-X(J_LEDGE,1)))
          CALL INTP(NIN,X_IN,P_IN,ARG,PSPEC_IM(J))
          WRITE(6,*) ' J,PSPEC_IM ', J, PSPEC_IM(J)
          PAVG_IM = PAVG_IM + PSPEC_IM(J)
 4904 CONTINUE
      END IF
C
C    DECIDE WHICH IS THE SUCTION SURFACE AND WHICH THE PRESSURE SURFACE.
      ISUCT = 1
      IF(PAVG_I1.GT.PAVG_IM) ISUCT = IM
C
C     INPUT THE CONTROL FACTORS FOR THE INVERSE MODE.
C
      READ(11,*) A2_SPEC_DEG
      READ(11,*) ROT_FAK
      READ(11,*) ANG_FAK
      READ(11,*) R_FAK_I1, R_FAK_IM
      READ(11,*) N_SMTH , F_SMTH
      READ(11,*) NINV_START, NINV_END, NINV_UPP
      JSET_TK = 5
      READ(11,*) FTK_SCALE, FTK_MIN, TK_ADD_LE, TK_ADD_TE, RF_THICK,
     &           JSET_TK
 4111 CONTINUE
      READ(11,*) PADD_I1, PADD_IM
C
      FAK_SCL  = 2.0
      A2_SPEC  = A2_SPEC_DEG*DEGRAD
      A2_NOW   = A2_SPEC
      A2_COMP  = A2_SPEC
      A2_COMP_NEW = A2_SPEC
      A2_SET   = A2_SPEC
      CONLIM   = 1.0E-6
C
      WRITE(6,*)  ' INVERSE DESIGN CONTROL FACTORS '
      WRITE(6,*)
      WRITE(6,*) ' A2_SPEC_DEG',         A2_SPEC_DEG
      WRITE(6,*) ' ROT_FAK    ',         ROT_FAK
      WRITE(6,*) ' ANG_FAK    ',         ANG_FAK
      WRITE(6,*) ' R_FAK_I1, R_FAK_IM',  R_FAK_I1, R_FAK_IM
      WRITE(6,*) ' N_SMTH , F_SMTH    ',  N_SMTH , F_SMTH
      WRITE(6,*) ' NINV_START, NINV_END, NINV_UPP',
     &             NINV_START, NINV_END, NINV_UPP
      WRITE(6,*) ' FTK_SCALE, FTK_MIN, TK_ADD_LE, TK_ADD_TE, RF_THICK',
     &             FTK_SCALE, FTK_MIN, TK_ADD_LE, TK_ADD_TE, RF_THICK
      WRITE(6,*) ' JSET_TK = ', JSET_TK

      WRITE(6,*) ' FAK_SCL =              ', FAK_SCL
C
      S_MERID(1) = 0.0
      DO J = 1,JM
          TKORIG(J)    = RT_THICK(J,1)
          TKSET(J)     = 0.0
          TKMIN(J)     = 0.0
          TKLIN(J)     = 0.0
          FSOURCE(J)   = 1.0
...
C
          IF(J.GT.1) THEN
              XD   = X(J,1) - X(J-1,1)
              RD   = R(J,1) - R(J-1,1) 
              S_MERID(J) = S_MERID(J-1) + SQRT(XD*XD + RD*RD)
          END IF
C
          XD1       = X(J,2)  - X(J,1)
          RD1       = R(J,2)  - R(J,1)
          TK_SURF(J)   = SQRT(XD1*XD1 + RD1*RD1)
      END DO
C
      DO J = J_LEDGE,J_TEDGE
          TKSET(J)     = TKORIG(J)*FTK_SCALE
          TKMIN(J)     = TKORIG(J)*FTK_MIN
      END DO 
C
      B_CHORD     = S_MERID(J_TEDGE) - S_MERID(J_LEDGE)
      WRITE(6,*) ' BLADE MERIDIONAL CHORD = ', B_CHORD
      TK_ADD_LE       = TK_ADD_LE*B_CHORD
      TK_ADD_TE       = TK_ADD_TE*B_CHORD
C
      DO J = 2,JM
      ABT_MID(J)  = ABTOT(IMID,J,1)
      END DO
C
      CALL SMOOTH(2, J_TEDGE, 10, 0.5, S_MERID, ABT_MID)
C
      SUM_ABT     = 0.0
      DO J = J_LEDGE+1,J_TEDGE 
      JJ   = J + 1 - J_LEDGE
      SUM_ABT     = SUM_ABT + ABT_MID(J)
      FRAC_CRD(J) = (S_MERID(J) - S_MERID(J_LEDGE))/B_CHORD
      END DO
C
C     SET A LINEAR VARIATION OF THICKNESS WITH MERIDIONAL CHORD.
      JSET_TK   = J_LEDGE + JSET_TK
C
      DO J = J_LEDGE, J_TEDGE
      FDIST  = (FRAC_CRD(J)       - FRAC_CRD(JSET_TK))
     &        /(FRAC_CRD(J_TEDGE) - FRAC_CRD(JSET_TK))
C      TKLIN(J) = 1.0 - FDIST*FDIST
      TKLIN(J) = 1.0 - FDIST
      END DO
C
C  FIT AN ELLIPSE AT THE LEADING EDGE
      DO J   = J_LEDGE, JSET_TK
      FLE    = (FRAC_CRD(JSET_TK) - FRAC_CRD(J))
     &        /(FRAC_CRD(JSET_TK) - FRAC_CRD(J_LEDGE))
      TKLIN(J) = SQRT(1.0 - FLE*FLE)
      END DO
C
C      WRITE(6,*) ' TK_ADD_LE,  TK_ADD_TE', TK_ADD_LE, TK_ADD_TE
C     SET THE SPECIFIED BLADE THICKNESS.
      DO J = J_LEDGE, J_TEDGE
      TKSET(J) = TKSET(J) + TK_ADD_LE*TKLIN(J)
     &         +            TK_ADD_TE*FRAC_CRD(J)
C      WRITE(6,*) 'J,TKORIG =', TKORIG(J),'TKLIN = ',J,TKLIN(J),
C     &            ' TKSET = ', TKSET(J)
      END DO
C
      DO J = J_LEDGE, J_TEDGE
      PSPEC_I1(J) = PSPEC_I1(J) + 2*PADD_I1*(0.5 -FRAC_CRD(J))
      PSPEC_IM(J) = PSPEC_IM(J) + 2*PADD_IM*(0.5 -FRAC_CRD(J))
      END DO 
C
      DO J = J_LEDGE+1, J_TEDGE
      FRAC_ABT(J) =  ABT_MID(J)/SUM_ABT
C      WRITE(6,*) ' J, ABT_M(D,  FRAC_ABT(J) ', J,ABT_MID(J), FRAC_ABT(J)
      END DO
C
      CLOSE(11)
C
      WRITE(6,*)
      WRITE(6,*) ' INVERSE MODE DATA READ IN OK.'
      WRITE(6,*)
C
C**********************************************************************
C**********************************************************************
...
C$OMP&PRIVATE(I,J,K,RONOW,EKE,TSTATIC,ESTAT,UNOW,PO_RO,
C$OMP&HNOW,PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
C$OMP&SHARED(RO,ROE,P,HO,VR,VT,VX,T_STATIC,ROSUB,
C$OMP&RGAS_NOW,GA_PV,PHEXP,WET,H_TO_T,ENTPY,
C$OMP&IM,JM,KM,CV,CP,RGAS,IFGAS,ITIMST,
C$OMP&TREF,EREF,ET1,ET2,ET3,ET4,
C$OMP&PO_REF,RO_REF,RF_PTRU,RF_PTRU1,DENSTY,
C$OMP&VSOUND,DP_DRO,VS_VMAX,RF_VSOUND,
C$OMP&VMAX,VS_NEW,FRACDIF,DVSOUND)
      IF(ITIMST.LT.5) THEN
C
      IF(IFGAS.EQ.0) THEN
...
C$OMP DO SCHEDULE(STATIC)
     ...
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
           ...
           RO(I,J,K)        = RONOW
           T_STATIC(I,J,K)  = TNOW 
           RGAS_NOW(I,J,K)  = PO_RO/TNOW
           GA_PV(I,J,K)     = GA_PV_NOW
           PHEXP(I,J,K)     = PO_RO/HNOW
...
C
C --- Unified PARALLEL region for all MASS FLUX loops ---
C$OMP PARALLEL DEFAULT(NONE)
C$OMP&PRIVATE(I,J,K,AVGRVX,AVGRVR,AVGRVT,AVGRO,AVGFLUX)
C$OMP&SHARED(KM,KMM1,JM,IM,IMM1)
C$OMP&SHARED(ROVX,ROVR,ROVT,RO)
C$OMP&SHARED(AQX,AQR,ABX,ABT,ABR)
C$OMP&SHARED(FLOWX,FLOWT,XFLUX,TFLUX)
...
C$OMP DO SCHEDULE(STATIC)
 ...
 5025 CONTINUE
C$OMP END DO
C     implicit barrier: DO 5030 reads ROWT
C
C$OMP DO SCHEDULE(STATIC)
      DO 5030 K=1,KMM1
      DO 5030 J=2,JM
 ...
 5030 CONTINUE
C$OMP END DO
C     implicit barrier: DO 5050 reads FLOWT
C
...
C**********************************************************************
C 
C   START THE INVERSE MODE
C
C**********************************************************************
C**********************************************************************
C
C     CALCULATE AND WRITE OUT THE CALCULATED SURFACE PRESSURES OF THE
...
C
      DO J = J_LEDGE,J_TEDGE
      NJ   = J - J_LEDGE + 1
      P_SSURF(NJ) = 0.5*(P(1,J,1)  + P(1,J,2))
      P_PSURF(NJ) = 0.5*(P(IM,J,1) + P(IM,J,2))  
      SFRAC(NJ)   = (S_MERID(J)- S_MERID(J_LEDGE))
     &            /(S_MERID(J_TEDGE) - S_MERID(J_LEDGE))
      END DO
C
      NON_B = J_TEDGE - J_LEDGE + 1
C
      CALL SMOOTH(1,NON_B,10,0.5,SFRAC,P_SSURF)
      CALL SMOOTH(1,NON_B,10,0.5,SFRAC,P_PSURF)
C
      NSURF = 21
      WRITE(8,*) IFINV_I1, IFINV_IM, ' IFINV_I1, IFINV_IM '
C
      WRITE(8,*) NSURF, ' NIN , I = I SURFACE'
C
      DO NPTS = 1,NSURF
      FSURF   = FLOAT(NPTS-1)/FLOAT(NSURF-1)
      CALL INTP(NON_B, SFRAC, P_SSURF, FSURF, PS_START)
      WRITE(8,5701) FSURF, PS_START
      END DO
      WRITE(8,*) NSURF, 'NIN, I = IM SURFACE'
      DO NPTS = 1,NSURF
      FSURF   = FLOAT(NPTS-1)/FLOAT(NSURF-1)
      CALL INTP(NON_B, SFRAC, P_PSURF, FSURF, PP_START)
      WRITE(8,5701) FSURF, PP_START
      END DO
 5701 FORMAT(F10.3,F15.0)
C
      WRITE(8,*) A2_SPEC_DEG       , '  A2_SPEC IN DEG '
      WRITE(8,*) ROT_FAK           , '  ROT_RAK'
      WRITE(8,*) ANG_FAK           , '  ANG_FAC '
      WRITE(8,*) R_FAK_I1, R_FAK_IM, '  RFAC_I1, RFAC_IM'
      WRITE(8,*) N_SMTH , F_SMTH   , '  NSMTH, FSMTH '
      WRITE(8,*) NINV_START, NINV_END, NINV_UPP,
     &          'NINV_START, NINV_END, NINV_UPP' 
      WRITE(8,5702) FTK_SCALE, FTK_MIN, TK_ADD_LE/B_CHORD, 
     &              TK_ADD_TE/B_CHORD, RF_THICK,(JSET_TK-J_LEDGE)
 5702 FORMAT(5F8.4,I5,' TK_SCALE, TK_MIN, TK_ADD_LE, TK_ADD_TE, RF_THICK
     &, JSET_TK')
      WRITE(8,*) 0.0, 0.0,   '    PADD_I1, PADD_IM '
C
      CLOSE(8)
C
...
C**********************************************************************
C
C     CALCULATE THE SOURCE TERM ON THE BLADE SURFACE
C
C  SET THE SPECIFIED PRESSURES EQUAL THE CURRENT PRESSURE IF NOT USING
C  THE INVERSE MODE ON A BLADE SURFACE. ALSO SET "P_AVGG" .
C
      DO 4931 J = 1,JM
      ROVMER(J)    = SQRT(ROVX(IMID,J,1)*ROVX(IMID,J,1) +
     &                    ROVR(IMID,J,1)*ROVR(IMID,J,1) ) 
      EKEREL = VR(IMID,J,1)*VR(IMID,J,1) + WT(IMID,J,1)*WT(IMID,J,1) 
     &       + VX(IMID,J,1)*VX(IMID,J,1)
      TSTAT  = T_STATIC(IMID,J,1)
      TOREL  = TSTAT + 0.5*EKEREL/CP  
      IF(IFINV_I1.EQ.0)  PSPEC_I1(J) = P(1,J,1)
      IF(IFINV_IM.EQ.0)  PSPEC_IM(J) = P(IM,J,1)
      IF(J.GT.J_TEDGE.OR.J.LT.J_LEDGE) PSPEC_I1(J) = P(1,J,1)
      IF(J.GT.J_TEDGE.OR.J.LT.J_LEDGE) PSPEC_IM(J) = P(IM,J,1)
      PSPEC_AVG = 0.5*(PSPEC_I1(J) + PSPEC_IM(J) )   
      POREL     = P(IMID,J,1)*(TOREL/TSTAT)**RFGA      
      P_AVGG(J) =  POREL
 4931 CONTINUE
C
C******************************************************************************
C******************************************************************************
C     SET THE SOURCE ON THE I = 1 SURFACE
C
      IF(IFINV_I1.GT.0)  THEN
C
C******************************************************************************
C******************************************************************************
 ...
 5066 CONTINUE
C
      JSET         = (J_LEDGE + J_TEDGE)/2       
      FFLUX        = R_FAK_I1*ROVMER(JSET)
      SUM_SOURCE   = 0.0
C
      DO 5062 J= J_LEDGE+1,J_TEDGE
      P_AVG_I1  = 0.25*(P(1,J,1)+P(1,J,2)+P(1,J-1,1)+P(1,J-1,2))
      PSPEC_AVG = 0.5*(PSPEC_I1(J) + PSPEC_I1(J-1)) 
      PERR     = (PSPEC_AVG - P_AVG_I1)/P_AVGG(J)            
      F_SCALE  =  FAK_SCL/(1. + (FAK_SCL - 1.0)*ABS(PERR) ) 
      SURF_FLUX(J)  = PERR*F_SCALE*FFLUX*ABT_MID(J)
      SUM_SOURCE    = SUM_SOURCE + SURF_FLUX(J)
 5062 CONTINUE
C    
      TE_SOURCE = SUM_SOURCE  
C
C     IF THIS IS THE PRESSURE SURFACE.
      IF(ISUCT.EQ.IM) THEN
           TE_NOW    =  RT_THICK(J_TEDGE,1)
           TK_SET    =  TKSET(J_TEDGE)
           TE_SOURCE =  TE_SOURCE 
     &   - (TK_SET - TE_NOW)*ROVMER(J_TEDGE)*TK_SURF(J_TEDGE)
      END IF
C
      DO 5063 J= J_LEDGE+1,J_TEDGE
      SRF_FLUX         =  SURF_FLUX(J)
      FDIST            =  FRAC_ABT(J)
      TE_FLUX          =  TE_SOURCE*FDIST
      TOT_FLUX         =  SRF_FLUX - TE_FLUX
      SUM_SOURCE_I1(J) =  SUM_SOURCE_I1(J-1) + TOT_FLUX 
C
C     CHANGE THE SURFACE MASS FLUX IN RESPONSE TO THE PRESSURE DIFFERENCES..
C
          TFLUX(1,J,1)  =  TOT_FLUX
C
      IF(SRF_FLUX.GT.0.0) THEN
          ROVX(1,J,1)  = ROVX(2,J,1)
          ROVX(1,J,2)  = ROVX(2,J,2)
          RORVT(1,J,1) = RORVT(2,J,1)
          RORVT(1,J,2) = RORVT(2,J,2)
          ROVR(1,J,1)  = ROVR(2,J,1)
          ROVR(1,J,2)  = ROVR(2,J,2)
          HO(1,J,1)    = HO(2,J,1)
          HO(1,J,2)    = HO(2,J,2)
      END IF 
C
 5063 CONTINUE
C
C   SET THE SOURCE TERM BEHIND THE TRAILING EDGE.
C
           DO 5067 J = J_TEDGE+1,JM
           SUM_SOURCE_I1(J) = SUM_SOURCE_I1(J_TEDGE)
 5067      CONTINUE
C
C******************************************************************************
C******************************************************************************
      END IF
C
C     END OF SETTING TFLUX ON THE I = 1 SUFACE
C
C******************************************************************************
C******************************************************************************
C     SET THE SOURCE ON THE I = IM SURFACE.
c     NOTE THAT THE SOURCE TERMS ARE POSITIVE FOR OUTWARDS FLOW FROM THE BLADE.
C     WHICH MEANS THAT THE  "TFLUX" IS NEGATIVE.
C
      IF(IFINV_IM.GT.0)  THEN
C
C******************************************************************************
...
C
      JSET      = (J_LEDGE + J_TEDGE)/2          
      FFLUX     = R_FAK_IM*ROVMER(JSET)
      SUM_SOURCE= 0.0
C
      DO 6062 J = J_LEDGE+1,J_TEDGE 
      P_AVG_IM  = 0.25*(P(IM,J,1)+P(IM,J,2)+P(IM,J-1,1)+P(IM,J-1,2))
      PSPEC_AVG = 0.5*(PSPEC_IM(J) + PSPEC_IM(J-1)) 
      PERR      = (PSPEC_AVG- P_AVG_IM)/P_AVGG(J)       
      F_SCALE   = FAK_SCL/(1. + (FAK_SCL - 1.0)*ABS(PERR) ) 
      SURF_FLUX(J)  = PERR*F_SCALE*FFLUX*ABT_MID(J)
      SUM_SOURCE    = SUM_SOURCE + SURF_FLUX(J)   
 6062 CONTINUE
...
C
C
      DO 6063 J = J_LEDGE+1,J_TEDGE 
      SRF_FLUX         =  SURF_FLUX(J)
      FDIST            =  FRAC_ABT(J)
      TE_FLUX          =  TE_SOURCE*FDIST
      TOT_FLUX         =  SRF_FLUX - TE_FLUX
      SUM_SOURCE_IM(J) =  SUM_SOURCE_IM(J-1) + TOT_FLUX

C
C     CHANGE THE SURFACE MASS FLUX IN RESPONSE TO THE PRESSURE DIFFERENCES.
C
          TFLUX(IM,J,1)  = -TOT_FLUX
C 
      IF(SRF_FLUX.GT.0.0) THEN
          ROVX(IM,J,1)  = ROVX(IMM1,J,1)
          ROVX(IM,J,2)  = ROVX(IMM1,J,2)
          RORVT(IM,J,1) = RORVT(IMM1,J,1)
          RORVT(IM,J,2) = RORVT(IMM1,J,2)
          ROVR(IM,J,1)  = ROVR(IMM1,J,1)
          ROVR(IM,J,2)  = ROVR(IMM1,J,2)
          HO(IM,J,1)    = HO(IMM1,J,1)
          HO(IM,J,2)    = HO(IMM1,J,2)
      END IF
C
 6063 CONTINUE
C
C   SET THE SOURCE TERM BEHIND THE TRAILING EDGE.
C
           DO 6060 J = J_TEDGE+1,JM
           SUM_SOURCE_IM(J) = SUM_SOURCE_IM(J_TEDGE)
 6060      CONTINUE
C
C******************************************************************************
C******************************************************************************
C
      END IF
C
C     END OF SETTING TFLUX ON THE I = IM SURFACE
C******************************************************************************
C******************************************************************************
C
C   WRITE OUTPUT FROM THE INVERSE MODE EVERY 500 STEPS
C
C   CALCULATE AND WRITE OUT THE AVERAGE DIFFERNCE BETWEEN SPECIFIED AND
C   ACHIEVED SURFACE PRESSURES.
C
      IF(MOD(NSTEP,500).EQ.0)  THEN
C
      IF(IFINV_I1.GT.0) THEN
      WRITE(6,*) ' INVERSE MODE FOR BLADE SURFACE NUMBER ', 1
      AVG_PFORCED = 0.0
      DO 6066 J = 2,JM
      PFORCED     = PSPEC_I1(J) - P(1,J,1)
      AVG_PFORCED = AVG_PFORCED + PFORCED
 6066 CONTINUE
C
      WRITE(6,*)
      WRITE(6,*)' AVERAGE PRESSURE ERROR (P-PSPEC) ON I = 1 SURFACE = ',
     &  -AVG_PFORCED/(J_TEDGE - J_LEDGE),'N/M**2'
       WRITE(6,*) ' AVERAGE FRACTIONAL PRESSURE ERROR=',
     &  -AVG_PFORCED/(J_TEDGE - J_LEDGE)/(PO1(1) - PDOWN_HUB)
      WRITE(6,*)
C
      END IF
C
      IF(IFINV_IM.GT.0) THEN
C
      WRITE(6,*) ' INVERSE MODE FOR BLADE SURFACE NUMBER ', IM
      AVG_PFORCED = 0.0
      DO 6067 J = 2,JM
      PFORCED     = PSPEC_IM(J) - P(IM,J,1)
      AVG_PFORCED = AVG_PFORCED + PFORCED
 6067 CONTINUE
C
      WRITE(6,*)
      WRITE(6,*)' AVERAGE PRESSURE ERROR (P-PSPEC) ON I=IM SURFACE = ',
     &  -AVG_PFORCED/(J_TEDGE - J_LEDGE),'N/M**2'
       WRITE(6,*) ' AVERAGE FRACTIONAL PRESSURE ERROR=',
     & -AVG_PFORCED/(J_TEDGE - J_LEDGE)/(PO1(1) - PDOWN_HUB)
      WRITE(6,*)
...
C******************************************************************************
C******************************************************************************
C   END OF INVERSE OUTPUT EVERY 500 STEPS
C
      END IF
C
C******************************************************************************
C******************************************************************************
C
C      NOW SET THE FLUXES ON SOLID SURFACES AND SET THE MASS FLOWS ON THE SURFACES
C
      DO 6064 J = J_LEDGE+1,J_TEDGE
      IF(IFINV_I1.EQ.0) TFLUX(1,J,1)  = 0.0
      IF(IFINV_IM.EQ.0) TFLUX(IM,J,1) = 0.0
      FLOWT(1,J,1)  = TFLUX(1,J,1)
      FLOWT(IM,J,1) = TFLUX(IM,J,1)
 6064 CONTINUE
C
C******************************************************************************
C******************************************************************************
C
C     IF USING INVERSE MODE .  CALL GEOM_MOD TO ADJUST THE BLADE SHAPE
C
      IF((NSTEP.GT.NINV_START.AND.NSTEP.LE.NINV_END).
     &              AND.MOD(NSTEP,NINV_UPP).EQ.0 )  CALL GEOM_MOD
C
C******************************************************************************
C******************************************************************************
...
C   END Q3D
      ...
      END IF
C$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)
C$OMP&PRIVATE(I,J,K,AVGRVX,AVGRVR)
C$OMP&SHARED(K1,K2,JM,IMM1)
C$OMP&SHARED(ROVX,ROVR,ASX,ASR,FLOWR,RFLUX)
      DO 5040 K=K1,K2
      DO 5040 J=2,JM
      DO 5040 I=1,IMM1
...
C
C     OpenMP: 4 main energy flux loops write different arrays
C     (XFLUX, SOURCE, RFLUX, TFLUX) - all fully independent.
C     NOWAIT on all; END PARALLEL barrier ensures completion.
C
C$OMP PARALLEL DEFAULT(NONE)
...
C$OMP END PARALLEL
C
C  TFLOW ADDITION (serial - conditional, small)
      IF(IM.EQ.2) THEN
      DO 5506 K=1,KMM1
      DO 5506 J=2,JM
...
C
C$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)
C$OMP&PRIVATE(J,K) SHARED(KMM1,JM,IM,IND,TFLUX)
      DO K=1,KMM1
      DO J=2,JM
      IF(IND(J).NE.1) THEN
...
C     NEXT FEW LINES MODIFIED BY JDD MAY 2021 .
      IF(PLATE_LOSS.GT.0.001) THEN
           DO 5999 K=1,KM
           ROVMERID  = (ROVX(IMID,JM,K)*DX(JM,K)
     &              +  ROVR(IMID,JM,K)*DR(JM,K))/DS(JM,K)
           ROVMSQ(K) = ABS(ROVMERID)*ROVMERID/RO(IMID,JM,K)
 5999      CONTINUE
      ENDIF
C
...
C
      IF(IFGAS.EQ.3) THEN
           GAMOUT = GA_PV(IMID,JMM1,K)
           RGAS   = RGAS_NOW(IMID,JMM1,K)
      END IF
C
      SUM     = 0.0
      DO 6010 I=1,IMM1
      ...
      TAVGOUT = TAVGOUT   + 
     &       0.5*(T_STATIC(I,JM-10,K) + T_STATIC(I+1,JM-10,K))*FP(I)
      PAVGOUT = PAVGOUT   + 
     &       0.5*(P(I,JM-10,K) + P(I+1,JM-10,K))*FP(I)
      ROAVGOUT = ROAVGOUT   + 
     &       0.5*(RO(I,JM-10,K) + RO(I+1,JM-10,K))*FP(I)
      SUM     = SUM     + 0.5*FP(I)*
...
C
      DO I=1,IM
C      PXTRAP(I) = P(I,JMM1,K) + FP_XTRAP*0.5*(P(I,JMM1,K) - P(I,JM-3,K))
      PXTRAP(I) = P(I,JMM1,K) + FP_XTRAP*(P(I,JMM1,K) - P(I,JMM2,K))
      END DO
      PXT_AVG  = 0.0
      DO I=1,IMM1
      PXT_AVG  = PXT_AVG + 0.5*(PXTRAP(I) + PXTRAP(I+1))*FP(I) 
      END DO
C
...
C
      DO 6018 I = 1,IM
C
      PADDI        = PXTRAP(I) - PXT_AVG
      DPWAVE       = FRACWAVE * (PD(K) + PADDI - P(I,JM,K))
      CSOUND       = SQRT(GAMOUT*PAVGOUT/ROAVGOUT)
      DROWAVE      = DPWAVE/(CSOUND*CSOUND)
      ROOLD        = RO(I,JM,K)
      DEWAVE       = P(I,JM,K)*DROWAVE/ROOLD/ROOLD
      ...
      T_EXIT       =   PNEW/(RGAS*RONEW)
      VMERSQ       =   VX(I,JM,K)*VX(I,JM,K) + VR(I,JM,K)*VR(I,JM,K)
      VEXIT        =   SQRT(VMERSQ)
      EKE          =   0.5*(VMERSQ + VT(I,JM,K)*VT(I,JM,K))
      EINT         =   ROE(I,JM,K)/ROOLD 

      IF(IFGAS.EQ.0.OR.IFGAS.EQ.3) THEN
        ROE(I,JM,K) = ROE(I,JM,K) + EINT*DROWAVE + ROOLD*(DEWAVE 
     &              + VEXIT*DVWAVE)
        HO(I,JM,K)  = (ROE(I,JM,K) + PNEW) /RONEW 
      END IF
C 
      IF(IFGAS.EQ.1) THEN
           HSTAT        = HFROMT(T_EXIT,TREF,HREF,CP1,CP2,CP3)
           HO(I,JM,K)   = HSTAT + EKE
           ROE(I,JM,K)  = RONEW*(HSTAT -  PNEW/RONEW + EKE) 
      END IF
C
 6018 CONTINUE
C
...
C
           IF(IFGAS.EQ.3) THEN
              HEXIT        = HO(I,JM,K) - EKE
              TEXIT        = HEXIT*H_TO_T(I,JM,K)
              RO(I,JM,K)   = PEXIT/(RGAS_NOW(I,JM,K)*TEXIT)
              ROE(I,JM,K)  = RO(I,JM,K)*HO(I,JM,K) - PEXIT
           END IF
C
...
C
      ...
      ELSE
C
C          IF ITIMST  LESS THAN  5
C
           RO_STAG = ROSTAG_IN(K)
           RO_IN   = RO(I,1,K)
           IF(RO_IN.GT.0.99999*RO_STAG) RO_IN = 0.99999*RO_STAG
           RO(I,1,K) = RO_IN
C
      IF(IFGAS.EQ.0.OR.IFGAS.EQ.3) PNEW = PO1(K)*(RO_IN/RO_STAG)**GA
C
      IF(IFGAS.EQ.1) THEN
           RORAT  = RO_IN/RO_STAG
     ...
     &              EPS1,EPS2)
      ...
      END IF
C
C      UPDATE THE STATIC PRESSURE AT INLET.
C
           IF(IN_PRESS.EQ.0) P(I,1,K) = RFIN*PNEW + RFIN1*P(I,1,K)
C
           IF(IABS(IN_PRESS).EQ.1) P(I,1,K) =
     &     RFIN1*PNEW  + RFIN*(2*P(I,2,K) -P(I,3,K))
C
           IF(IABS(IN_PRESS).EQ.3) P(I,1,K) = 
     &     RFIN1*PNEW + RFIN*(2*PAVG2 - PAVG3)
C
           IF(IABS(IN_PRESS).EQ.4) P(I,1,K) =
     &     RFIN1*PNEW + RFIN*(2*P(IMID,2,KMID) - P(IMID,3,KMID))
C
C     END IF ITMIST LESS THAN 5 LOOP
      END IF
C
C
...
C
             IF(IFGAS.EQ.3) THEN
                  XPON= 0.5*(PO1(K)/(HO_INN(K)*RO_STAG)+PHEXP(I,1,K))
                  HSTAT     = HO_INN(K)*(P(I,1,K)*RPO1(K))**XPON
                  TSTATIC   = P(I,1,K)/RO_IN/RGAS
                  HSTAG     = HO_INN(K)
                  IF(HSTAT.GT.0.9999*HSTAG) HSTAT = 0.9999*HSTAG
                  VABSQ   = 2.0*(HSTAG -HSTAT)
                  IF(VABSQ.LT.1.0) VABSQ = 1.0
             END IF       
C
C    NOW IF ITIMST = 6 
      ELSE
...
C     END OF ITIMST  NE 6  LOOP
      END IF
C
C     NOW UPDATE THE VELOCITIES AT INLET
C
...
C
           RO(I,1,K)    = RO_IN
           ROVT(I,1,K)  = RO_IN*VT(I,1,K)
           ROVR(I,1,K)  = RO_IN*VR(I,1,K)
           RORVT(I,1,K) = ROVT(I,1,K)*R(1,K)
           ROVX(I,1,K)  = RO_IN*VX(I,1,K)
C
           IF(IFGAS.EQ.0) THEN
                ROE(I,1,K)   = P(I,1,K)/GA1 + RO_IN*0.5*VABSQ
   	        HO(I,1,K)    = CP*TO1(K) 
          END IF
C
          IF(IFGAS.EQ.1) THEN
                ROE(I,1,K)   = RO_IN*HSTAG - P(I,1,K)
                HO(I,1,K)    = HSTAG
           END IF
C
          IF(IFGAS.EQ.3) THEN

               ROE(I,1,K) = RO_IN*HO_INN(K) - P(I,1,K)
               HO(I,1,K)  = HO_INN(K)
         END IF
                
C     END OF ITIMST LT 5 LOOP
      END IF
C
C******************************************************************************
C******************************************************************************
...
C$OMP&SOURCE,XFORCE,PEFF,VX,AQX,FLOWX,XFLUX,
C$OMP&ASX,FLOWR,RFLUX,FLOWT,ABX,TFLUX,
C$OMP&BFORCE_X,IND,
C$OMP&NCOOLB,JCBS,JCBE,KCBS,KCBE,IC,VXCWLI1,VXCWLIM,
C$OMP&NCOOLW,JCWS,JCWE,ICWS,ICWE,KC,VXCWLK1,VXCWLKM,
C$OMP&IFSHROUD,SHROUDVX)
C$OMP DO SCHEDULE(STATIC)
      DO 6100 K=1,KMM1
      DO 6100 J=1,JM
C$OMP SIMD
...
C$OMP&SHARED(KMM1,KM,JM,IMM1,IM,
C$OMP&R,VT,TFORCE,FLOWX,SOURCE,XFLUX,
C$OMP&RAVG_CELL,PEFF,FLOWT,ABT,TFLUX,
C$OMP&FLOWR,RFLUX,BFORCE_T,IND,
C$OMP&NCOOLB,JCBS,JCBE,KCBS,KCBE,IC,RVTCWLI1,RVTCWLIM,
C$OMP&NCOOLW,JCWS,JCWE,ICWS,ICWE,KC,RVTCWLK1,RVTCWLKM,
C$OMP&IFSHROUD,SHROUDRVT)
C$OMP DO SCHEDULE(STATIC)
      DO 6200 K=1,KMM1
      DO 6200 J=1,JM
...
C******************************************************************************
      CALL TIMER_START(T_LOOP_MOM_FLUX)
      CALL TIMER_START(T_MOMF_VR_DO)
C
C$OMP PARALLEL DEFAULT(NONE)
C$OMP&PRIVATE(I,J,K,NC,AVGVR,AVGP,AVGRVT,AVGVT,AVGP1,AVGPM,T1,T2)
C$OMP&SHARED(KMM1,KM,JM,IMM1,IM,
C$OMP&VR,PEFF,FLOWX,AQR,XFLUX,
C$OMP&FLOWT,ABR,TFLUX,FLOWR,ASR,RFLUX,
C$OMP&ROVT,VT,RFORCE,VOLOR,SOURCE,
C$OMP&BFORCE_R,IND,
C$OMP&NCOOLB,JCBS,JCBE,KCBS,KCBE,IC,VRCWLI1,VRCWLIM,
C$OMP&NCOOLW,JCWS,JCWE,ICWS,ICWE,KC,VRCWLK1,VRCWLKM,
C$OMP&IFSHROUD,SHROUDVR)
C$OMP DO SCHEDULE(STATIC)
      DO 6300 K=1,KMM1
      DO 6300 J=1,JM
C$OMP SIMD
...
C     Timing: end DO section, start SINGLE section
C$OMP MASTER
      CALL TIMER_STOP(T_MOMF_VR_DO)
      CALL TIMER_START(T_MOMF_VR_SGL)
C$OMP END MASTER
C$OMP BARRIER
C
C  TFLOW ADDITION
...
C    END OF OUTPUT FOR SHROUD LEAKAGE
      ENDIF
C
C     CALL GEOM_MOD TO PRINT OUT INVERSE DESGN DATA AT END OF CALCULATION
C
      IF(IFEND.EQ.1.AND.IF_INV.EQ.1)  CALL   GEOM_MOD
C
...
C
C******************************************************************************
C      EVERY 50 STEPS CHECK FOR POINTS OUTSIDE THE TABULATED DATA AND WRITE THEM OUT.
C
      IF(MOD(NSTEP,50).EQ.0.AND.IFGAS.EQ.3) THEN
C
      NRO_LOW = 0
      RO_LOW  = 1.0E6
      NRO_HI  = 0
      ...
      DO 9003 J = 1,JM
      RONOW = RO(I,J,K)
      EKE         = 0.5*(VR(I,J,K)*VR(I,J,K) + VT(I,J,K)*VT(I,J,K) 
     &                 + VX(I,J,K)*VX(I,J,K))
      UNOW  = ROE(I,J,K)/RONOW - EKE
      IF(RONOW.LT.ROMIN)  NRO_LOW = NRO_LOW + 1
      IF(RONOW.GT.ROMAX)  NRO_HI  = NRO_HI  + 1
      IF(UNOW.LT.UMIN)    NU_LOW  = NU_LOW  + 1
      IF(UNOW.GT.UMAX)    NU_HI   = NU_HI   + 1
      IF(RONOW.LT.RO_LOW)  RO_LOW = RONOW
      IF(RONOW.GT.RO_HI)   RO_HI  = RONOW
      IF(UNOW.LT.U_LOW)    U_LOW  = UNOW
      IF(UNOW.GT.U_HI)     U_HI   = UNOW
 9003 CONTINUE
      WRITE(6,9005) ROMIN, ROMAX, UMIN/1000.0, UMAX/1000.0
 9005 FORMAT(' ROMIN, ROMAX, UMIN, UMAX OF THE TABULATED DATA',T75,
     & 2F12.5,2F12.2)

      WRITE(6,9004) RO_LOW,RO_HI,U_LOW/1000.,U_HI/1000.
 9004 FORMAT(' MIN/MAX VALUES IN THE FLOW FIELD, RO_LOW, RO_HI, U_LOW, 
     &U_HI', T75, 2F12.5,2F12.2)

      WRITE(6,9006) NRO_LOW, NRO_HI, NU_LOW, NU_HI
 9006 FORMAT(' POINTS OUTSIDE TABLULATED INPUT DATA, N_ROLOW, N_ROHIGH, 
     & N_ULOW, N_UHIGH', T75, 4I12)
...
C
 ...
 9001 FORMAT(' STEP   EMAX AT I   J   K  EAVG    ECONT AT J=    VREF   V 
     &MAX  AT I   J   K  M_MAX AT I   J   K    IN FLOW      OUT FLOW   
     &RAT FLOW')
C
      RATFLOW = BLADE_FLOW(JM)/FLOWRAT
      WRITE(6,9000) NSTEP,EMAX,IMAX,JMAX,KMAX,EAVG,ECONT,JCONT,VREF,
     &              VMAX,IMAX,JMAX,KMAX,
     &              REL_M_MAX,IMACH,JMACH,KMACH,FLOWRAT,BLADE_FLOW(JM),
     &              RATFLOW
 9000 FORMAT(I5,F8.4,3I4,2F8.4,I6,F8.2,F8.2,3I4,F8.4,3I4,3F12.4,I10)
C
C******************************************************************************
C******************************************************************************
...
C
      WRITE(6,*) 'IN OUTPUT, WRITING THE GEOMETRY OUTPUT "grid_out" ' 
C
      WRITE(21) NSTEPS_MAX
      WRITE(21) IM,JM,KM
      WRITE(21) CP,GA
...
C 
   ...
   25 CONTINUE
C
      WRITE(6,*) 'IN OUTPUT, GEOMETRY FILE "grid_out" written, NSTEP= ',
     &            NSTEP
C
C
C******************************************************************************
...
C
C
C     CALCULATE THE ENTROPY FUNCTION. SAVED AS "STORE" .
C 
      DO 1200 I=1,IM
      DO 1200 K=1,KM
      DO 1200 J=1,JM
     ...
     &           +VR(I,J,K)*VR(I,J,K))
      WREL = SQRT(VX(I,J,K)*VX(I,J,K) + WT(I,J,K)*WT(I,J,K)
     &           +VR(I,J,K)*VR(I,J,K))
C
      IF(IFGAS.EQ.0) THEN
...
C
      IF(IFGAS.EQ.1) THEN
           ...
           V_SND = SQRT(GA*RGAS*TSTAT)
      END IF
C
      IF(IFGAS.EQ.3) THEN
           HSTAT    = HO(I,J,K) - EKE
           AVGEXP   = 0.5*(PHEXP(IMID,J,KMID) + PHEXP(IMID,1,KMID))
           PO_IDEAL = P(I,J,K)*(HO(IMID,1,KMID)/HSTAT)**(1.0/AVGEXP)
           STORE(I,J,K) = PO_IDEAL/PO1(KMID)
           V_SND = SQRT(GA_PV(I,J,K)*P(I,J,K)/RO(I,J,K))
      END IF
C
         M_REL(I,J,K) = WREL/V_SND
C
 1200 CONTINUE
...
C     WRITE OUT THE TRANSFORMED VISCOSITY IF USING THE SA MODEL
      ...
      ELSE
C
C     IF IFGAS = 0 or 1,  WRITE OUT THE TURBULENT/LAMINAR VISCOSITY
C     RATIO WHICH IS USED FOR PLOTTING ONLY.
          IF(IFGAS.EQ.0.OR.IFGAS.EQ.1)
     &    WRITE(7) (((VISC_RAT(I,J,K),I=1,IM),J=1,JM),K=1,KM)
C
C     IF  IFGAS = 3  WRITE OUT THE PRESSURE. WHICH IS USED FOR PLOTTING ONLY.
          IF(IFGAS.EQ.3)
     &    WRITE(7) (((P(I,J,K),I=1,IM),J=1,JM),K=1,KM)
      END IF

C      IF "IFGAS = 3, WRITE OUT THE TRUE ENTROPY
       IF(IFGAS.EQ.3) WRITE(7) (((ENTPY(I,J,K),I=1,IM),J=1,JM),K=1,KM)
C
C      IF "IFGAS" = 0 or 1   "STORE" IS THE ENTROPY FUNCTION EXP(-S/R)
       IF(IFGAS.EQ.0.OR.IFGAS.EQ.1) WRITE(7)
     &        (((STORE(I,J,K),I=1,IM),J=1,JM),K=1,KM)
C
C     WRITE OUT EXTRA PLOTTING DATA IF USING STEAM.
C
      IF(IFGAS.EQ.3) THEN
      WRITE(7) (((GA_PV(I,J,K),I=1,IM),J=1,JM),K=1,KM)
      WRITE(7) (((WET(I,J,K),I=1,IM),J=1,JM),K=1,KM)
      WRITE(7) (((H_TO_T(I,J,K),I=1,IM),J=1,JM),K=1,KM)
      END IF
C
...
C
      IF(IFGAS.EQ.3) THEN
          HSTAG   = HO(I,J,K)
          HSTAT   = HSTAG - EKE
          TSTATIC = HSTAT*H_TO_T(I,J,K)
          TSTAG   = HSTAG*H_TO_T(I,J,K)
          PSTAG   = P(I,J,K)*(HSTAG/HSTAT)**(1/PHEXP(I,J,K))
          ROSTAG  = PSTAG/(TSTAG*RGAS_NOW(I,J,K))
          TEMP4(I,J,K) = TSTAG
          V_SONIC = SQRT(GA_PV(I,J,K)*P(I,J,K)/RO(I,J,K) )
      END IF                  
C
...
C  END Q3D
C
      WRITE(6,*) ' INPUT STYLE, ANS_IN = ', ANS_IN
C
      IF(ANS_IN.EQ.'n'.OR.ANS_IN.EQ.'N')THEN
...
C
     ...
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
      ...
      END IF
C 
C     NOW SET GUESS OF PRESSURE AND TEMPERATURE FOR ALL  J VALUES.
C         
      DO 3210 J=1,JM
C
           PS   = PGUESS(J)
...
C
     ...
     &            BALPHA1,BALPHA2)
      ...
      END IF
C
C    ESTIMATE THE INTERNAL ENERGY  UINT(J,K) FROM A PROCESS WITH POLYTROPIC EFFICIENCY = ETAG.
C
      IF(IFGAS.EQ.3) THEN
C
          RONOW  = ROSTAG_IN(K)*PRAT**(1.0/GA_PV_AVG)            
          IF(J.EQ.1) THEN
                     DU = (PGUESS(1) - PO1(K))/RONOW/GA_PV_IN
          ELSE
                     DU = (PGUESS(J) - PGUESS(J-1))/RONOW/GA_PV_NOW
          END IF
C
          ETAG  = 0.9
...
C
      IF(IFGAS.EQ.0)   HO(I,1,K) = CP*TO1(K)
      IF(IFGAS.EQ.1)   HO(I,1,K) = HFROMT(TO1(K),TREF,HREF,CP1,CP2,CP3)
      IF(IFGAS.EQ.3)   HO(I,1,K) = HO_INN(K)
C
C******************************************************************************
      FACJ = 0.9
      DO 3320 J = 2,JM
...
C
     ...
     &         *AQR(IMID,J,KAVG))*NBLADE(J)
      IF(J.EQ.2) AIN = AREA
      VS        = VM1(KAVG)*AIN*RO(IMID,2,KAVG)/(AREA*RO(IMID,J,KAVG))
...
C 
      ...
      IF(IFGAS.EQ.3) THEN
C
C    ITERATE TO GET THE INITIAL DENSITY AND PRESSURE AT ALL
C    GRID POINTS IF USING TABULATED GAS PROPERTIES.
C     
...
C
      ...
      VT(I,1,K) = VT(I,2,K)
      IF(IFGAS.EQ.3) THEN
              T_STATIC(I,1,K) = T_STATIC(I,2,K)
              RO(I,1,K)    = RO(I,2,K)
              GA_PV(I,1,K) = GA_PV(I,2,K)
              ENTPY(I,1,K) = ENTPY(I,2,K)
              WET(I,1,K)   = WET(I,2,K)
              TEMP1(I,1,K) = TEMP1(I,2,K)
      END IF
C	
 3310 CONTINUE
 3300 CONTINUE
C
C     REPEAT THE VELOCITY ESTIMATE WITH THE NEW DENSITY IF IFGAS = 3.
C
      IF(IFGAS.EQ.3.AND.NLOOP.EQ.1) GO TO 3290
C
C
C***************************************************************************** 
      DO 3215 J = 1,JM
C
      IF(IFGAS.EQ.3) THEN
C
          WRITE(6,3321) J, T_STATIC(IMID,J,KMID), P(IMID,J,KMID),
     &                  RO(IMID,J,KMID),VX(IMID,J,KMID),VR(IMID,J,KMID),
     &                  VT(IMID,J,KMID),WET(IMID,J,KMID),
     &                ENTPY(IMID,J,KMID)/1000.,TEMP1(IMID,J,KMID)/1000., 
     &                HO(IMID,J,KMID)/1000.
      ELSE
C
          WRITE(6,3322) J, T_STATIC(IMID,J,KMID), P(IMID,J,KMID),
     &                  RO(IMID,J,KMID),VX(IMID,J,KMID),VR(IMID,J,KMID),
     &                  VT(IMID,J,KMID),HO(IMID,J,KMID)/1000.0
C 
      END IF
C      
 3322 FORMAT('J= ',I5,' INITIAL GUESS OF: T, P ,RO, VX, VR, VT, HO ',
     &        F10.2, F12.1, 5F10.3)
 3321 FORMAT('J= ',I5,' INITIAL GUESS OF: T, P ,RO, VX, VR, VT,WETNESS,
     & ENTROPY,ENERGY,HSTAG',  F10.2, F12.1, 8F10.3)
C
 3215 CONTINUE
C
C******************************************************************************
...
C  
      IF(IFGAS.EQ.0) ROE(I,J,K) = P(I,J,K)/(GA-1) + EKE*RO(I,J,K)
C
      IF(IFGAS.EQ.1) THEN
      ...
      END IF
C
      IF(IFGAS.EQ.3) ROE(I,J,K) = RO(I,J,K)*(UINT(J,K) + EKE)
C
...
C
      ...
      YPLUS_IM(J,K) = 25.0
      SPAREVAR(I,J,K) = 1.0
C      IF(I.EQ.IMID.AND.K.EQ.KMID) THEN
C      WRITE(6,*) ' INITIAL GUESS OF ROVX,ROVR,ROVT',
C     &              ROVX(I,J,K),ROVR(I,J,K),ROVT(I,J,K)
C      END IF
 3500 CONTINUE
C
...
C
           ...
           GA_PV(I,J,K)    = GA_PV_NOW
           ENTPY(I,J,K)    = ENTPY_NOW
           HO(I,J,K)       =  UNOW + PNOW/RONOW + EKE
           WET(I,J,K)      = 1.0 - DRYNOW
      END IF
C
C
      IF(ITIMST.LT.5) ROSUB(I,J,K) = RO(I,J,K)
C
      IF(ITIMST.GE.5) P(I,J,K) = PO_REF - DP_DRO*(RO_REF - ROSUB(I,J,K))
C
      PEFF(I,J,K)  = P(I,J,K)
C
C    END OF SETTING UP THE FLOW FROM A RESTART FILE
 3900 CONTINUE
C
C**********************************************************************************
C**********************************************************************************
...
C
      IF(ITIMST.LT.5) THEN
C
           IF(IFGAS.EQ.0)  VSOUND = SQRT(RGAS*GA*TO1(KMID))
C
           IF(IFGAS.EQ.1) THEN
     ...
     &                + CP3*(TO1(KMID)-TREF)*(TO1(KMID)-TREF)
           ...
           END IF
C
           IF(IFGAS.EQ.3) VSOUND = 
     &     SQRT(GA_PV(IMID,1,KMID)*PO1(KMID)/ROSTAG_IN(KMID))

      END IF
C
      WRITE(6,*) ' INITIAL VALUE OF VSOUND = ', VSOUND
      WRITE(6,*)
C
C     SET THE TIME STEP BASED ON THE LOCAL GRID DIMENSION AND THE SPEED OF SOUND
C     AT THE INLET BOUNDARY.
...
C 
     ...
     &                  SUMTSTAT,SUMPSTAT,SUMHO,SUMWET,SUMHSTAT) 
C
...
C
     ...
     &             + P(I+1,J,K+1) + P(I,J,K+1))
      SUMHO(J)     = SUMHO(J) + DFLOW4*(HO(I,J,K)+HO(I+1,J,K)
     &             + HO(I+1,J,K+1)+HO(I,J,K+1))
      SUMWET(J)    = SUMWET(J) + DFLOW4*(WET(I,J,K)+WET(I+1,J,K)
     &             + WET(I+1,J,K+1)+WET(I,J,K+1))
      SUMHSTAT(J)  = SUMHSTAT(J) + DFLOW4*(STORE(I,J,K)+STORE(I+1,J,K)
...
C
      DO 400 J=1,JM
      ROVM=SQRT(ROVX(IMID,J,KMID)*ROVX(IMID,J,KMID)+ROVR(IMID,J,KMID)
     &  *ROVR(IMID,J,KMID))
      DELTA = (AVFLOW/FLOW(J) -1.0)*ROVM
      DELTA = DELTA*RFLOW
      DO 400 I=1,IM
      DO 400 K=1,KM
      ROVM = SQRT(ROVX(I,J,K)*ROVX(I,J,K)+ROVR(I,J,K)*ROVR(I,J,K))
C
C  ADDED   3/9/90 may not be valid for radial flow machines
C
      IF(ROVX(I,J,K).LT.0.0)  ROVM = -ROVM
C
      ROVX(I,J,K) = ROVX(I,J,K) + ROVX(I,J,K)/ROVM*DELTA
      ROVR(I,J,K) = ROVR(I,J,K) + ROVR(I,J,K)/ROVM*DELTA
      RO_WT       = ROVT(I,J,K) - UBLADE(J,K)*RO(I,J,K)
      ROVT(I,J,K) = ROVT(I,J,K) + RO_WT/ROVM*DELTA
C
  400 CONTINUE
C
      RETURN
...
C
     ...
     &          TEMPP(JD),VLAM_L(MAXKI),VTURB_L(MAXKI)
C
C
...
C
C$OMP PARALLEL DEFAULT(NONE)
C$OMP&PRIVATE(NRW,J1,JROW,JTRHUB,JTRTIP,JTRLOW,JTRUP)
C$OMP&PRIVATE(JLEDGE,JTEDGE,WREL,I,K,WABSQ)
C$OMP&PRIVATE(PERP,RE,RELOG,ROUGH)
C$OMP&PRIVATE(TWALLK1,TWALLKM,TWALLI1,TWALLIM)
C$OMP&PRIVATE(CF,CFLAM,REKSQ,REK)
C$OMP&PRIVATE(A1,A2,A3,VSTAR,YPLSK1,YPLSKM,FMULT)
C$OMP&PRIVATE(DPERP,FYPLUS,XFAC,RMAX,RATVIS,VISTOT)
C$OMP&PRIVATE(XGEN,RGEN,TGEN,TAVGG,QGEN,SGEN_NOW)
C$OMP&PRIVATE(PERPK1,PERPKM,PERPI1,PERPIM)
C$OMP&PRIVATE(YPLUS_OLD,YPLUS_NEW)
C$OMP&PRIVATE(PLUSK,RELOG10,FUNCN,CF_FULL_ROUGH)
C$OMP&PRIVATE(YPLSI1,YPLSIM,YPLSP,IMAX)
C$OMP&PRIVATE(FACJ,FBLEND,ISUB,IM1,IP1)
C$OMP&PRIVATE(DELP_BLADE,TAU_REF,RHO_REF,VEL_REF)
C$OMP&PRIVATE(PITCH,SREF)
C$OMP&PRIVATE(VXAVG,VRAVG,WTAVG,WABS,ROAVG,XSTRES)
C$OMP&PRIVATE(RAVG,RSTRES,TSTRES,AREA,WBOUND,WTB)
C$OMP&PRIVATE(VTAVG,TAVG,WVISC,QFLOW,VLAM_L,VTURB_L)
C$OMP&FIRSTPRIVATE(VISLAM,TCOND,FTCOND)
C$OMP&SHARED(NROW,JSTART,JTRAN_K1,JTRAN_KM,JTRAN_I1)
C$OMP&SHARED(JTRAN_IM,JLE,JTE)
C$OMP&SHARED(WRAD,ASX,ASR,ABX,ABR,ABT)
C$OMP&SHARED(VX,VR,VT,WT,RO,T_STATIC)
C$OMP&SHARED(R,VOL,RAVG_CELL,WHUB,WTIP,DWALLSQ)
C$OMP&SHARED(DPDS_CELL,IM,IMM1,KM,KMM1,JM,IMID,KMID)
C$OMP&SHARED(CP,PRANDTL,REYNO)
C$OMP&SHARED(ROUGH_H,ROUGH_T,ROUGH_L,ROUGH_U)
C$OMP&SHARED(FKLAM,FKTURB,FILAM,FITURB,FIWAKE,FIUP)
C$OMP&SHARED(FMIXUP,RF_VIS,RF_VIS1)
C$OMP&SHARED(CFWALL,YPLUSWALL,YPLAM,YPTURB,IBOUND)
C$OMP&SHARED(CHORD,FTRANS,IFPRINT,IFEND)
C$OMP&SHARED(KTIPS,KTIPE,IND,NBLADE,NROWS)
C$OMP&SHARED(XFORCE,RFORCE,TFORCE,QSOURCE)
C$OMP&SHARED(VISC_RAT,Y_PLUS,SGEN,TEMP4)
C$OMP&SHARED(YPLUS_K1,YPLUS_KM,YPLUS_I1,YPLUS_IM)
C$OMP&SHARED(VISCOSY,ROVEXIT,REYNOLDS)
C$OMP&SHARED(PO1,PDOWN_HUB,PDOWN_TIP,RGAS,TO1)
C
C --- Phase 1: Streamwise (K=const) faces ---
C$OMP DO SCHEDULE(STATIC)
      DO 50 J=2,JM
C
      NRW    = NROW(J)
...
C    
      ...
      VLAM_L(K)  = FKLAM(NRW,K)*VISLAM/DPERP
      VTURB_L(K) = FKTURB(NRW,K)*ROAVG(K)*ABS(WABS(K+1)-WABS(K-1))
     &           * FMIXUP
      VTURB_L(K) = VTURB_L(K)*FYPLUS
   35 CONTINUE
...
C     CALCULATE THE STRESSES ON THE STREAMWISE FACES OF THE ELEMENTS.
      DO 41 K=2,KMM1
      FMULT     = AREA(K)*(VLAM_L(K)+VTURB_L(K))
      XSTRES(K) = FMULT*(VXAVG(K+1)-VXAVG(K-1))
      RSTRES(K) = FMULT*(VRAVG(K+1)-VRAVG(K-1))
      VISTOT    = VISLAM*(1.0 + VTURB_L(K)/VLAM_L(K))
      VISC_RAT(I,J,K) = VTURB_L(K)/VLAM_L(K)
...
C
C --- Phase 2: Bladewise (I=const) faces ---
C$OMP DO SCHEDULE(STATIC)
      DO 100 J=2,JM
C
      NRW    = NROW(J)
...
C    reduce the turbulent viscosity in the sublayer and buffer region.
      ...
      VLAM_L(I)  = FILAM(NRW,I)*VISLAM/DPERP
      VTURB_L(I) = FITURB(NRW,I)*ROAVG(I)*ABS(WABS(I+1)-WABS(I-1))
     &           * FMIXUP
      VTURB_L(I) = VTURB_L(I)*FYPLUS
   55 CONTINUE
...
C
      ...
      VLAM_L(I)    = FILAM(NRW,I)*VISLAM/DPERP
      VTURB_L(I)   = FBLEND*ROAVG(I)*ABS(WABS(IP1)-WABS(IM1))
      VISC_RAT(I,J,K) = VISC_RAT(I,J,K) + VTURB_L(I)/VLAM_L(I)
      FMULT        = AREA(I)*(VLAM_L(I)+VTURB_L(I))
...
C
      ...
      DPERP       = VOL(ISUB,J,K)/AREA(I)
      VLAM_L(I)   = FILAM(NRW,I)*VISLAM/DPERP
      VTURB_L(I)  = FMIXUP*FIUP(NRW,I)*ROAVG(I)
     &            * ABS(WABS(IP1)-WABS(IM1))
      VISC_RAT(I,J,K)= VISC_RAT(I,J,K) + VTURB_L(I)/VLAM_L(I)
      FMULT       = AREA(I)*(VLAM_L(I)+VTURB_L(I))
      XSTRES(I)   = FMULT*(VXAVG(IP1)-VXAVG(IM1))
      RSTRES(I)   = FMULT*(VRAVG(IP1)-VRAVG(IM1))
...
C    INTERPOLATE FOR THE STAGNATION TEMPERATURES AT THE GRID POINTS.
  ...
  130 CONTINUE
C
      WRITE(6,*)'STAGNATION TEMPERATURE AT POINTS ON INLET BOUNDARY'
      WRITE(6,10)(TO1(K),K=1,KM)
C
      IF(IFGAS.EQ.3) THEN
C    INTERPOLATE FOR THE STAGNATION ENTHALPY AT THE GRID POINTS.
      DO 121 K=1,KM
           CALL INTP(KIN,SUMF_IN,HO_INN,SUMFN(K),ANS(K))
  121 CONTINUE
      DO 131 K=1,KM
           HO_INN(K)=ANS(K)
  131 CONTINUE
C
      WRITE(6,*) 'STAGNATION ENTHALPY AT GRID POINTS ON INLET BOUNDARY.'
      WRITE(6,11)(HO_INN(K),K=1,KM)
C
      END IF
C
C    INTERPOLATE FOR THE STAGNATION DENSITY AT THE GRID POINTS.
      DO 122 K=1,KM
           CALL INTP(KIN,SUMF_IN,ROSTAG_IN,SUMFN(K),ANS(K))
  122 CONTINUE
      DO 132 K=1,KM
           ROSTAG_IN(K)=ANS(K)
  132 CONTINUE
C
      WRITE(6,*)  'STAGNATION DENSITY AT GRID POINTS ON INLET BOUNDARY.'
      WRITE(6,10)(ROSTAG_IN(K),K=1,KM)
C
...
C******************************************************************************C
C
      SUBROUTINE GRID_DOWN(K,J1,J2,S1,S2,NGAP,SDIS_T,XINT,RINT,NEXTRAP,
...
C
      DO 30 J = J1,J2
      CALL INTP(NGAP,SDIS_T,XINT,SNEW(J),ANSX(J))
      CALL INTP(NGAP,SDIS_T,RINT,SNEW(J),ANSR(J))
   30 CONTINUE
      DO 31 J = J1,J2
      XSURF(J,K)   = ANSX(J)
      RSURF(J,K)   = ANSR(J)
...
C
      ...
      DO 200 I = 1,IMM1
      DO 200 J = J3+1,J4
      ASABS    = SQRT(ASX(I,J,KSHRD)*ASX(I,J,KSHRD) 
     &         + ASR(I,J,KSHRD)*ASR(I,J,KSHRD))
      ASTOT    = ASTOT + ASABS
      POUTAVG  = POUTAVG + ASABS*(P(I,J,KSHRD) + P(I+1,J,KSHRD)
     &         + P(I,J-1,KSHRD) + P(I+1,J-1,KSHRD))
      AS(I,J)  = ASABS
  200 CONTINUE
...
C     AND CALCULATE POAX THE STAGNATION PRESSURE NEGLECTING SWIRL VELOCITY
C
      DO 250 J=J1,J2
      DO 250 I=1,IM
      VMERSQ    = 0.5*(VX(I,J,KSHRD)*VX(I,J,KSHRD)
     &          + VR(I,J,KSHRD)*VR(I,J,KSHRD))
      EKE       = VMERSQ + 0.5*VT(I,J,KSHRD)*VT(I,J,KSHRD)
C
      IF(IFGAS.EQ.0) THEN
           TSTAT     = (HO(I,J,KSHRD) - EKE)/CP
           TOREL     = TSTAT      + VMERSQ/CP
           POAX(I,J) = P(I,J,KSHRD)*(TOREL/TSTAT)**RFGA
           TOAX(I,J) = TOREL
      END IF
C
      IF(IFGAS.EQ.1) THEN
           ...
           POAX(I,J) = P(I,J,KSHRD)*PRAT
           TOAX(I,J) = TOREL
      END IF
C
      IF(IFGAS.EQ.3) THEN
           HSTAT      = HO(I,J,KSHRD) - EKE
           HOAX(I,J)  = HSTAT     + VMERSQ
           POAX(I,J)  = P(I,J,KSHRD)*(HOAX(I,J)/HSTAT)
     &               **(1.0/PHEXP(I,J,KSHRD))
           TOAX(I,J)  = HOAX(I,J)*H_TO_T(I,J,KSHRD)
      END IF
C
  250 CONTINUE
C
     ...
     &        ASABS*(TOAX(I,J)+TOAX(I+1,J)+TOAX(I,J-1)+TOAX(I+1,J-1))
      HOAXAVG = HOAXAVG +
     &        ASABS*(HOAX(I,J)+HOAX(I+1,J)+HOAX(I,J-1)+HOAX(I+1,J-1))
      HOAVG = HOAVG + ASABS*(HO(I,J,KSHRD) + HO(I+1,J,KSHRD)
     &      + HO(I,J-1,KSHRD) + HO(I+1,J-1,KSHRD))

      VTAVG = VTAVG + ASABS*(VT(I,J,KSHRD) + VT(I+1,J,KSHRD)
     &      + VT(I,J-1,KSHRD) + VT(I+1,J-1,KSHRD))
      VRAVG = VRAVG + ASABS*(VR(I,J,KSHRD) + VR(I+1,J,KSHRD)
     7     + VR(I,J-1,KSHRD) + VR(I+1,J-1,KSHRD))
      VXAVG = VXAVG + ASABS*(VX(I,J,KSHRD) + VX(I+1,J,KSHRD)
     &     + VX(I,J-1,KSHRD) + VX(I+1,J-1,KSHRD))
      AS(I,J) = ASABS
 300  CONTINUE
C
      ALEAKOUT  = ASTOT
...
C
      IF(IFGAS.EQ.3) THEN
           JAVG = (J3+J4)/2
           HSTATAVG  = HOAXAVG*(PRAT**PHEXP(IMID,JAVG,KSHRD))
           VJET      = SQRT(2.0*(HOAXAVG - HSTATAVG))
           TJET      = HSTATAVG*H_TO_T(IMID,JAVG,KSHRD)
           RGAS      = RGAS_NOW(IMID,JAVG,KSHRD)
      END IF
C
C            
      ...
      CONTRACT   = 0.6
      ALEAK      = 2*PI*R(JSHROUD,KSHRD)*SEALGAP(NR)*CONTRACT
...
C
      FK  = 1.0
      RFSHRD1 = 1.-RFSHRD
      IF(KSHRD.EQ.1) FK = -1.0
      FACGR    = FK*SHROUDFLBL/ALEAKOUT
      RIN      = 0.5*(R(J4,KSHRD) + R(J3,KSHRD))
      ROUT     = 0.5*(R(J1,KSHRD) + R(J2,KSHRD))
...
C
      ...
      DO 9554 J=J1,J2
      DO 9554 I=1,IMM1
 9554 RFLUX(I,J,KSHRD) = RFLUX(I,J,KSHRD) + SFLUX(I,J)
      DO 9553 J=J3,J4
      DO 9553 I=1,IMM1
 9553 RFLUX(I,J,KSHRD) = RFLUX(I,J,KSHRD) + SFLUX(I,J)
 9555 CONTINUE
C
      RETURN
      END
...
C
  ...
  610 CONTINUE
C
      JAV  = (JCBS(NCB) + JCBE(NCB))/2
      KAV  = (KCBS(NCB) + KCBE(NCB))/2
      IAV  = IC(NCB)
C
...
C
C
C      CALCULATE THE VELOCITY OF THE INJECTED COOLANT.
C
      IF(IFGAS.EQ.1) THEN
           DELT        = TOIN-TREF
...
C
      IF(IFGAS.EQ.3) THEN  
          GANOW  = GA_PV(IAV,JAV,KAV)          
          HOIN   = TOIN/H_TO_T(IAV,JAV,KAV)
          HOREL  = HOIN  + 0.5*VBLADE*VBLADE - W_PRESWIRL 
          PO_REL = POIN*(HOREL/HOIN)**(1.0/PHEXP(IAV,JAV,KAV))
          HOABS  = HOREL + 0.5*VBLADE*VBLADE 
          HSTAT  = HOREL/(1. + 0.5*(GANOW-1.)*MACHCOOL(NCB)*
     &                   MACHCOOL(NCB))
         TSTAT  = HSTAT*H_TO_T(IAV,JAV,KAV)
         TOREL  = HOREL*H_TO_T(IAV,JAV,KAV)
         TIN    = TSTAT
         VIN    = SQRT(2.0*(HOREL - HSTAT))
      END IF
C
...
C
  ...
  710 CONTINUE
C
      JAV = (JCWS(NRW) + JCWE(NRW))/2
      IAV = (ICWS(NCW) + ICWE(NCW))/2
      KAV = KC(NCW)
C
...
C
C      CALCULATE THE VELOCITY OF THE COOLANT AS EJECTED.
C
      IF(IFGAS.EQ.0) THEN
           TOREL  = TOIN  + (0.5*VWALL*VWALL - W_PRESWIRL)/CP
           PO_REL = PO_IN*(TOREL/TOIN)**RFGA
...
C
      IF(IFGAS.EQ.1) THEN
           ...
           TOREL       = TFROMH(HOREL,TREF,HREF,HT1,HT2,HT3,HT4)
           PO_REL      = PO_IN*(TOREL/TOIN)**RFGAGAS
           TIN         = TOREL/(1. + 0.5*(GAGAS-1.)*MACHCOOL(NCB)*
     &                   MACHCOOL(NCB)) 
           HSTAT       = HFROMT(TIN,TREF,HREF,CP1,CP2,CP3) 
           IF(HSTAT.GT.HOREL) HSTAT = 0.99999*HOREL
...
C
      IF(IFGAS.EQ.3) THEN
          ...
          HOIN   = TOIN/H_TO_T(IAV,JAV,KAV)  
          HOREL  = HO_IN  + 0.5*VWALL*VWALL - W_PRESWIRL
          PO_REL = PO_IN*(HOREL/HOIN)**(1.0/PHEXP(IAV,JAV,KAV))
          HOABS  = HOREL + 0.5*VWALL*VWALL 
          HSTAT  = HOREL/(1. + 0.5*(GANOW-1.)*MACHCOOL(NCB)*
     &                   MACHCOOL(NCB))
         TSTAT  = HSTAT*H_TO_T(IAV,JAV,KAV)
...
C
     ...
     &          CHECK_FLOW(JD),SUMHO(JD),SUMWET(JD),SUMHSTAT(JD)
C
...
C
C
      IF(IFGAS.EQ.0) PSTAG = PSTAT*(TSTAG/TS)**RFGA
C
      IF(IFGAS.EQ.1) THEN
            TRAT  = TSTAG/TS
      ...
      END IF
C
      IF(IFGAS.EQ.3) PSTAG = PSTAT*(HSTAG/HSTAT)**(1.0/PHEXP(I,J,K)) 
C
C******************************************************************************
C     CALCULATE THE ENTROPY FUNCTION. EXP(-S/RGAS) . 
C     THIS IS ONLY APPROXIMATE FOR OTHER THAN PERFECT GASES.
C
      TRAT    = TS/TO1(KMID)
      PRAT    = PSTAT/PO1(KMID)
      ENTRPY  = 0.0
...
C
      IF(IFGAS.EQ.0) THEN
           TRU_ENT = CP*LOG(TRAT)- RGAS*LOG(PRAT)
           ENTRPY   = EXP(-TRU_ENT/RGAS)
           TEMP4(I,J,K) = TRU_ENT
      END IF
C
      IF(IFGAS.EQ.1) THEN
           ...
           TRU_ENT = -RGAS*ALOG(ENTRPY)
           TEMP4(I,J,K) = ENTRPY
      END IF
C
      IF(IFGAS.EQ.3) THEN
      TEMP4(I,J,K) = ENTPY(I,J,K)
      END IF          
C
 5605 CONTINUE
C
...
C******************************************************************************
     ...
     &            SUMPSTAT,SUMHO,SUMWET,SUMHSTAT) 
C
C******************************************************************************
C
      IF(IFEND.EQ.1)  THEN
...
C
      ...
      WRITE(11)   (SUMHO(J), J=1,JM)
      WRITE(11)   (SUMRVT(J),J=1,JM)
      WRITE(11)   (SUM_ENTPY(J),J=1,JM)
      WRITE(11)   (SUMWET(J), J=1,JM)
      WRITE(11)   (SUMHSTAT(J), J=1,JM)
C    END OF IFEND = 1 LOOP
      ENDIF
C********************************************************
...
C
      ...
      SUMHO(J)      = SUMHO(J)/TFLOW
      SUMWET(J)     = SUMWET(J)/TFLOW
      SUMHSTAT(J)   = SUMHSTAT(J)/TFLOW
 5611 CONTINUE
C
C    SET "IFSTAGE" TO DECIDE IF A SINGLE ROW OR A STAGE/MULTISTAGE CALCULATION.
C
      IFSTAGE = 0
      DHOREF  = ABS( SUMHO(1) - SUMHO(JM) )
      DHOUT   = ABS( SUMHO(JM) - SUMHSTAT(JM) )
      DHIN    = ABS( SUMHO(1)  - SUMHSTAT(1))
      DHREF   = AMAX1(DHIN,DHOUT)
      IF(DHOREF.LT.0.1*DHREF)THEN
           IFSTAGE  = 1
           DHOREF = DHREF
      END IF
      DO J = 1,JM
      ETA_LOSS(J) = SUMTO(JM)*(SUM_ENTPY(J)-SUM_ENTPY(1))/DHOREF
      END DO
C
C ********************************************************
...
C
      IF(IFSTAGE.EQ.0) THEN
           WRITE(6,5645)
           WRITE(6,5646)
C   OUTPUT FOR MULTIPLE BLADE ROWS
 5645 FORMAT( '    J           MASS          LOCAL/INLET         LOST
     &        STAGNATION      STAGNATION       MASS AVG        STAGNATIO
     &N')
 5646 FORMAT( '  VALUE      FLOW. KG/S          FLOW          EFFICIENCY
     &        PRESSURE        TEMPERATURE        R*VT            ENTHALP
     &Y          ENTROPY          WETNESS')
      ELSE
           WRITE(6,5647)
           WRITE(6,5648)
C   OUTPUT FOR A SINGLE BLADE ROW 
...
C
     ...
     &             SUMTO(J),SUMRVT(J),SUMHO(J)/1000.0,
     &             SUM_ENTPY(J)/1000., SUMWET(J)
 5630 FORMAT( I5, 9F17.5)
 5640 CONTINUE
C
C************************************************************************************
C
...
C
      IF(IFGAS.EQ.3) THEN
          PO_AT_HOLE = PS_EJECT*(TOCOOLB(NCB)/TS_EJECT)
     &              **(1.0/PHEXP(IAV,JAV,KAV))
      END IF
C
C
      POCOOL     = POCOOLB(NCB)
...
C
C      CALCULATE THE ISENTROPIC WORK AVAILABLE FROM THE COOLANT
C      AND THE TOTAL ENTHAPPY INJECTED
C
      IF(IFGAS.EQ.0) THEN
...
C
      IF(IFGAS.EQ.3) THEN
           HOCOOL    = TOCOOL/H_TO_T(IAV,JAV,KAV)
           AVGEXP    = 0.5*(PHEXP(IAV,JAV,KAV) + PHEXP(IMID,JM,KMID))
           HOCOOL_IS = HOCOOL*(PO2/POCOOL)**AVGEXP
           WCOOL_IS = WCOOL_IS + SUM_MAS*(HOCOOL - HOCOOL_IS)
           HO2IS_AT_HOLE = HOCOOL*(PO2/PO_AT_HOLE)**AVGEXP
           WCOOL_AT_HOLE = WCOOL_AT_HOLE
     &                  + SUM_MAS*(HOCOOL - HO2IS_AT_HOLE)
          ENTHIN = ENTHIN + SUM_MAS*HOCOOL
      END IF
...
C*****************************************************************************
C*****************************************************************************
C   NOW DO THE SAME FOR COOLANT EJECTED FROM THE END WALLS
C*****************************************************************************
C*****************************************************************************
C
      DO 160 NCW = 1,NCOOLW
...
C
      IF(IFGAS.EQ.3) THEN
          PO_AT_HOLE = PS_EJECT*(TOCOOLW(NCW)/TS_EJECT)
     &              **(1.0/PHEXP(IAV,JAV,KAV))
      END IF
...
C
      IF(IFGAS.EQ.3) THEN
           HOCOOL      = TOCOOL/H_TO_T(IAV,JAV,KAV)
           AVGEXP = (PHEXP(IAV,JAV,KAV) + PHEXP(IMID,JM,KMID))/2
           HOCOOL_IS   = HOCOOL*(PO2/POCOOL)**AVGEXP
           WCOOL_IS    = WCOOL_IS + SUM_MAS*(HOCOOL - HOCOOL_IS)
           HO2IS_AT_HOLE = HOCOOL*(PO2/PO_AT_HOLE)**AVGEXP
           WCOOL_AT_HOLE = WCOOL_AT_HOLE
...
C
      IF(IFGAS.EQ.3) THEN
          HO_IN  =  SUMHO(1)
          HO_OUT =  SUMHO(J2)
          AVGEXP =  (PHEXP(IMID,J1,KMID) + PHEXP(IMID,J2,KMID))/2
          HO2_IS =  HO_IN*(PO2/PO_IN)**AVGEXP
          WMAIN_IS = BLADE_FLOW(J1)*(HO_IN - HO2_IS)
          WNET     = BLADE_FLOW(J1)*HO_IN + ENTHIN - FLOW_OUT*HO_OUT
      END IF
C
...
C
      IF(IFGAS.EQ.3) THEN
           RGAS = RGAS_NOW(IMID,J1,KMID)
           GA   = GA_PV(IMID,J1,KMID)
           CP   = RGAS*(GA/(GA - 1.0))
      END IF
C
C
     ...
     &                ' GAMMA = ',GA,' R = ',RGAS
      IF(IFGAS.EQ.3) THEN
      WRITE(6,*) ' GA IS THE ISENTROPIC PRESSURE-DENSITY EXPONENT '
      WRITE(6,*) ' RGAS IS THE TRUE VALUE OF  P/(RHO*T) '
      WRITE(6,*) ' CP IS NOT A TRUE VALUE, IT IS SIMPLY CALCULATED FROM
     & "RGAS*(GA/(GA-1.0))" '
      END IF
C
...
C
     ...
     &IC EFFICIENCY= ', ETA_ATH_NOP
      WRITE(6,*)
      WRITE(6,*) '*****************************************************'
C     END OF IFCOOL = 1 LOOP
      END IF
C
 3333 CONTINUE
C
C    END OF IFCOOL NE 0 LOOP
      END IF
C
C****************************************************************************
C****************************************************************************
...
C
C     JDD CHANGE  2/7/07  TO INCLUDE THE MIXING LOSS IN THE EFFICIENCY OF THE UPSTREAM BLADE ROW.
C     FOR THE LAST STAGE THERE IS NO MIXING DOWNSTREAM. SO BASE THE EFFY ON THE LAST BUT ONE POINT.
C
              IF(NSTG.EQ.NSTAGES) J2M1 = JM -1 
C
C     END OF JDD ADDITION
...
C     JMIX_STAGE IS ONLY USED TO FIND THE STAGE REACTION.
      ...
      ELSE
C
C   FOR THE WHOLE MACHINE THERE IS NO MIXING DOWNSTREAM. SO BASE THE EFFY ON THE LAST BUT ONE POINT, J2M1.
C
              J1   = 2
              J2   = JM
              J2M1 = JM - 1
...
C
      ...
      HO_IN   = SUMHO(J1)
      HO_OUT  = SUMHO(J2M1)
      HSTAT_IN  =  SUMHSTAT(J1)
      HSTAT_OUT =  SUMHSTAT(J2M1)
      ENTPY_IN  =  SUM_ENTPY(J1)
      ENTPY_OUT =  SUM_ENTPY(J2M1)
      TSTAT_IN  =  SUMTSTAT(J1)
      TSTAT_OUT =  SUMTSTAT(J2M1)
      DELTO   = TO_OUT - TO_IN
      TORAT   = TO_OUT/TO_IN
      PORAT   = PO2/PO_IN
...
C
           TSTAG_OUT =  TSTAT_OUT + 
     &     (HO_OUT - HSTAT_OUT)*H_TO_T(IMID,J2M1,KMID)
           
           DHO_LOSS   = TSTAG_OUT*(ENTPY_OUT - ENTPY_IN)
           DHS_LOSS   = TSTAT_OUT*(ENTPY_OUT - ENTPY_IN)

           HO_OUT_IS = HO_OUT    - DHO_LOSS
           HS_OUT_IS = HSTAT_OUT - DHS_LOSS

           DELHO         = HO_OUT    - HO_IN
           DELHO_IS      = HO_OUT_IS - HO_IN
           DELH_TOT_STAT = HS_OUT_IS - HO_IN
           ...
           ETATS   = 1.0
           IF(DELHO.GT.0.0) ETATS = DELH_TOT_STAT/DELHO
           IF(DELHO.LT.0.0) ETATS = DELHO/DELH_TOT_STAT
           EPOLY = 1.0
      END IF
C
C
...
C
     ...
     &                   PSIN,PSOUT
      WRITE(6,*)      ' INLET AND EXIT STAGNATION TEMPERATURES= ',
     &                  TO_IN, TO_OUT
      WRITE(6,*)      ' INLET AND EXIT STATIC TEMPERATURES=     ',
     &                  TSTAT_IN, TSTAT_OUT
      WRITE(6,*)      ' INLET AND OUTLET MASS FLOW RATES      = ',
     &                  BLADE_FLOW(J1),BLADE_FLOW(J2M1)
      WRITE(6,*)      ' INLET AND OUTLET STAGNATION ENTHALPIES= ',
     &                  HO_IN/1000.  , HO_OUT/1000. 
      WRITE(6,*)      ' INLET AND OUTLET STATIC ENTHALPIES    = ',
     &                  HSTAT_IN/1000.  , HSTAT_OUT/1000. 
      WRITE(6,*)      ' INLET AND OUTLET ENTROPIES            = ',
     &                  ENTPY_IN/1000, ENTPY_OUT/1000. 
C
      IF(NROWS.GT.1.OR.ABS(WRAD(1)).GT.0.1) THEN
C
      WRITE(6,*)
     ...
     & ' TOTAL TO TOTAL ISENTROPIC EFFICIENCY       = ', ETAIS 
      IF(IFGAS.EQ.0.OR.IFGAS.EQ.3)  WRITE(6,*) 
     & ' TOTAL TO STATIC ISENTROPIC EFFICIENCY      = ', ETATS 
      IF(IFGAS.EQ.0)  WRITE(6,*)
     & ' TOTAL TO TOTAL POLYTROPIC EFFICIENCY       = ', EPOLY
      WRITE(6,*)
      WRITE(6,*)    ' THE EFFICIENCIES DO NOT ALLOW FOR COOLING FLOWS OR 
     & BLEED FLOWS'
      WRITE(6,*)
C      IF(IFGAS.EQ.3) THEN
C      WRITE(6,*) ' THE EFFICIENCIES ARE NOT VERY ACCURATE IF USING GAS P
C     &ROPERTIES FROM A LOOK UP TABLE AND THE STAGE/MACHINE PRESSURE RATI
C     &O IS HIGH.'
C      END IF
C
C   END OF NROWS.GT 1 LOOP
      END IF
C
C     EVALUATE THE STAGNATION PRESSURE LOSS COEFFICIENT F THIS IS A CASCADE
      IF(NROWS.EQ.1.AND.ABS(WRAD(1)).LT.0.1) THEN
C
      DPIN  = ABS(PO_IN - PSIN)
      DPOUT = ABS(PO2   - PSOUT)
      DENOM = AMAX1(DPIN,DPOUT)
      POLOSS = (PO_IN - PO2)/DENOM
      WRITE(6,*)
      WRITE(6,*) ' THIS IS A STATIONARY CASCADE '
      WRITE(6,*) ' STAGNATION PRESSURE LOSS COEFFICIENT = ', POLOSS
      WRITE(6,*)
C    END OF CASCADE LOSS CALCULATION
      END IF
C
      IF(NSTG.LE.NSTAGES) THEN
C
...
C     NOTE IT IS IMPORTANT TO BASE THE WORK ON ONLY THE INLET MASS FLOW BECAUSE
C     OTHERWISE SMALL CHANGES IN MASS FLOW CAN CAUSE LARGE CHANGES IN WORK
C     BECAUSE THE ENTHALPIES AR VERY LARGE NUMBERS.
C
           WORK = BLADE_FLOW(J1)*(HO_IN - HO_OUT)/1000.0
C
      WRITE(6,*)
     ...
     &     , WORK,' KILOWATTS.'
      WRITE(6,*)
C
C    END OF NROWS GT 1 OR ABS(WRAD(1)).GT.0.1 LOOP
      END IF
C
C********************************************************************************
C   END OF NSTAGE LE NSTAGES LOOP
      ELSE
C
C********************************************************************************
C
C     NOW CALCULATE RESULTS FOR THE WHOLE MACHINE, NSTG = NSTAGES + 1
C
C********************************************************************************
C
      IF(IFBLEED.EQ.0) HBLEDTOT = 0.0
C
           FLOW_OUT = BLADE_FLOW(1) - SUMBLEED(JM) + SUMCWL(JM)
           WORK = (BLADE_FLOW(1)*HO_IN  - FLOW_OUT*HO_OUT 
...
C
     ...
     &                  BLADE_FLOW(1)*HO_IN/1000.
           WRITE(6,*) ' OUTLET ENTHALPY FLUX, KW  = ', 
     &                  BLADE_FLOW(JMM1)*HO_OUT/1000.
...
C
     ...
     &         ' KILOWATTS. THIS INCLUDES BLEED FLOWS AND COOLING FLOWS'
      WRITE(6,*) 
C     END OF NROWS GT 1 LOOP
      END IF
C
...
C        
     ...
     & ED STATES OF THE FLUID AT MACHINE ENTRY AND EXIT.'
      ...
      END IF
C
C     END OF NSTG.LE NSTAGES LOOP
      END IF
C
 2222 CONTINUE
C
C     END OF IFCOOL = O LOOP
      END IF
C
C
      RETURN
...
C
      IF(IFGAS.EQ.3) THEN
C
           CALL TABSEARCH(ROMIX,EMIX,
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
C
           TMIX     = TNOW
           PMIX     = PNOW
           ROTHALPY = HMIX + 0.5*(WMIXSQ - UJP1*UJP1)
           GAGAS    = GA_PV_NOW
      END IF
C
      IF(ITIMST.EQ.5.OR.ITIMST.EQ.6) 
...
C*********************************************************************************************
C
C   NEXT THE ISENTROPIC VALUES AT JMIX+2 ASSUMING AN ISENTROPIC EXPANSION FROM
C   THE MIXING PLANE CONDITIONS TO  PSTAT  AT JMIX+2 .
C
      IF(IFGAS.EQ.0) THEN
...
C
      IF(IFGAS.EQ.3) THEN
           ESTATJP2  = ROE(I,JP2,K)/ROJP2 - 0.5*VELSQJP2
C
           CALL TABSEARCH(ROJP2,ESTATJP2,
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
C
           T_ACT_JP2 = TNOW
           PSTAT     = PNOW
           PRAT      = PSTAT/PMIX
           HSTAT_JP2 = ESTATJP2 + PNOW/ROJP2
           PH_EXP    = PNOW/(ROJP2*HSTAT_JP2)
           HISENT_JP2 = HMIX*PRAT**PH_EXP
           TISENT_JP2 = HISENT_JP2*TNOW/HSTAT_JP2
           HSTAT(I)   = HISENT_JP2 
           WIS_SQ_JP2  = 2*(ROTHALPY - HISENT_JP2 + 0.5*UNOWJP2*UNOWJP2)
           RGASNOW =  PNOW/(ROJP2*TNOW)
           ROISENT_JP2 = PSTAT/(RGASNOW*TISENT_JP2)
      END IF
C
C
...
C
      ...
      END IF
      IF(IFGAS.EQ.3) THEN
           WMACH_JP2(I) = WACT_JP2/SQRT(GA_PV_NOW*PSTAT/RO_ACT_JP2)
      END IF
C
...
C
      IF(IFGAS.EQ.3) THEN
              EISENT = HSTAT(I) - PISENT(I)/ROISENT(I) + 0.5*V_JP2_SQ
              ROE_JP2   = EISENT*ROISENT(I)
      END IF
C
C*****************************************************************************************
...
C$OMP SIMD
                  DO I=1,IM
                        ...
                        CURV  = AVERG - 0.5*D(I,J,K)
     &                        - 0.25*(D(I,J-2,K)+D(I,J+2,K))
                        STORE(I,J,K) = SFX1*D(I,J,K)
     &                        + SFX*(AVERG + FAC_4TH*CURV)
                  END DO
            END DO
      END DO
C$OMP END DO NOWAIT
...
C
      D(1,J,1)  = SFT1*D(1,J,1)
     &          + SFTH*(D(2,J,1)+D(1,J,2))
      D(IM,J,1) = SFT1*D(IM,J,1)
     &          + SFTH*(D(IMM1,J,1)+D(IM,J,2))
      D(1,J,KM) = SFT1*D(1,J,KM)
     &          + SFTH*(D(1,J,KMM1)+D(2,J,KM))
...
C$OMP DO SCHEDULE(STATIC)
      DO 8500 K=1,KM
      DO 8501 I=2,IMM1
      D(I,J,K) = SFEX1*D(I,J,K)
     &          +SFEXH*(D(I+1,J,K)+D(I-1,J,K))
 8501 CONTINUE
      D(1,J,K) = D(1,J,K)
     &          +SFEXIT*(2.*D(2,J,K)-D(NUM3,J,K)-D(1,J,K))
      D(IM,J,K)= D(IM,J,K)
     &          +SFEXIT*(2.*D(IMM1,J,K)-D(IMM2,J,K)
     &          -D(IM,J,K))
 8500 CONTINUE
C$OMP END DO
C
...
C$OMP DO SCHEDULE(STATIC)
      ...
      DO 8601 K=2,KMM1
      D(I,J,K) = SFEX1*D(I,J,K)
     &          +SFEXH*(D(I,J,K-1)+D(I,J,K+1))
 8601 CONTINUE
      D(I,J,1) = D(I,J,1)
     &          +SFEXIT*(2.*D(I,J,2)-D(I,J,3)-D(I,J,1))
      D(I,J,KM)= D(I,J,KM)
     &          +SFEXIT*(2.*D(I,J,KMM1)-D(I,J,KMM2)
     &          -D(I,J,KM))
 8600 CONTINUE
C$OMP END DO
C
...
C     SET THE FINAL MERIDIONAL POSITIONS OF THE GRID POINTS ON THE BLADE SURFACE
      DO J = JLEROW,JTEROW
      SSDIST(J) = S_SS(N_LE) + S_MERD*FRAC_CHORD(J)
      END DO
C
C    FIND THE FRACTION OF MERIDIONAL CHORD AT THE STREAM SURFACE POINTS INPUT
...
C     SET THE FINAL MERIDIONAL POSITIONS OF THE GRID POINTS UPSTREAM OF THE LE.
      DO J = 1,JLEROW-1
      SSDIST(J) = S_SS(N_LE) + FRAC_CHORD_UP*S_MERD*(FRAC_CHORD(J)-1.0)
      END DO
C
C    FIND THE FRACTION OF MERIDIONAL CHORD AT THE STREAM SURFACE POINTS INPUT
C    DOWNSTREAM OF THE TRAILING EDGE.
...
C     SET THE FINAL MERIDIONAL POSITIONS OF THE GRID POINTS DOWNSTREAM OF THE TE.
      DO J = JTEROW+1,JMROW
      SSDIST(J) = SSDIST(JTEROW) + FRAC_CHORD_DWN*S_MERD*FRAC_CHORD(J)
      END DO
C
...
C
      ...
      Q3DFORCE  = 1.0
      TKSS_REF  = 0.05
      READ(5,*, ERR = 11)    Q3DFORCE, TKSS_REF
   11 CONTINUE
      WRITE(6,*) ' Q3DFORCE= ',Q3DFORCE,
     &           ' REFERENCE SS THICKNESS= ',TKSS_REF
      READ(5,*)    NSS
...
C
      ...
      DO 10 J = J1+1, J2
           ...
           SDIS_T(JLOC) = SDIS_T(JLOC-1) + SQRT(XD*XD+RD*RD)
   10 CONTINUE
C
      NVAL = J2+1-J1
C
      SRANGE   = SDIS_T(NVAL) - SDIS_T(1)
      IF(J1.EQ.1) SRANGE1 = SRANGE
C
      DO 20  J = 1, NVAL
      SDIS_T(J) = (SDIS_T(J)-SDIS_T(J1))/SRANGE
   20 CONTINUE
C
C  SET THE INLET SS THICKNESS = 0.02 X THE MERIDIONAL LENGTH OGF THE FIRST ROW.
      TKSS1 = TKSS(1)
      DO 25 N=1,NSS
      TKSS(N) = TKSS_REF*TKSS(N)/TKSS1*SRANGE1
   25 CONTINUE
C
...
C   
      DO 30 J = 1,NVAL 
      CALL INTP(NSS,FRACSS,TKSS,SDIS_T(J),TK_SS(J))
   30 CONTINUE
C
C  SMOOTH THE STREAM SURFACE THICKNESS
C
      CALL SMOOTH(1,NVAL,4,0.25,SDIS_T,TK_SS)
C
      TK_SS(1)     = TK_SS(2)
...
C
     ...
     &     /( SRANGE*(SDIS_T(JLOCP1) - SDIS_T(JLOCM1)) )
...
C
C
C     NOTE THE TREATMENT OF COOLING FLOWS WITH TABULAR INPUT OF GAS
C     PROPERTIES (IFGAS = 3) IS ONLY APPROXIMATE.
C
      IF(IFGAS.EQ.3) THEN
            JAV  = (JCBS(NCB) + JCBE(NCB))/2.0
            KAV  = (KCBS(NCB) + JCBE(NCB))/2.0
            IAV  = IC(NCB)
            RGAS  = RGAS_NOW(IAV,JAV,KAV)
            GAGAS = 1.0/(1.0 - PHEXP(IAV,JAV,KAV))
            CPGAS = RGAS/PHEXP(IAV,JAV,KAV)
            FGAGAS = (GAGAS - 1.0)/GAGAS
      END IF
C
      TOIN        = TOCOOLB(NCB)
...
C
      IF(IFGAS.EQ.3) THEN
            JAV  = (JCWS(NCW) + JCWE(NCW))/2.0
            KAV  =  KC(NCW)
            IAV  = (ICWS(NCB) + ICWE(NCW))/2.0
            RGAS  = RGAS_NOW(IAV,JAV,KAV)
            GAGAS = 1.0/(1.0 - PHEXP(IAV,JAV,KAV))
            CPGAS = RGAS/PHEXP(IAV,JAV,KAV)
            FGAGAS = (GAGAS - 1.0)/GAGAS
...
C**********************************************************************
C**********************************************************************
C  START THE BLADE GEOMETRY MODIFICATION IF USING THE INVERSE BLADE DESIGN OPTION
C**********************************************************************
C**********************************************************************
      SUBROUTINE GEOM_MOD
C
...
C******************************************************************************
C
      WRITE(6,*) ' ENTERED GEOM_MOD'
C
C   THE FIRST PART OF THE SUBROUTINE CALCULATES THE CURRENT EXIT FLOW
C   CONDITIONS AND HENCE THE COMPATIBLE EXIT FLOW ANGLE AND BLADE NUMBER
C   AND WRITES THEM OUT.
C
C   CALCULATE THE CURRENT EXIT FLOW CONDITIONS.
C
      VX_OUT = 0.0 
      VX_IN  = 0.0
      VR_IN  = 0.0
...
C
      DO I = 1,IMM1
      VX_IN   = VX_IN  - 0.5*FLOWX(I,2,1)*(VX(I,2,1)+VX(I+1,2,1))
      VX_OUT =VX_OUT - 0.5*FLOWX(I,JMM2,1)*(VX(I,JMM2,1)+VX(I+1,JMM2,1))
      VR_IN   = VR_IN  - 0.5*FLOWX(I,2,1)*(VR(I,2,1)+VR(I+1,2,1))
      VR_OUT =VR_OUT - 0.5*FLOWX(I,JMM2,1)*(VR(I,JMM2,1)+VR(I+1,JMM2,1))
      VT_OUT =VT_OUT - 0.5*FLOWX(I,JMM2,1)*(VT(I,JMM2,1)+VT(I+1,JMM2,1))
      VT_IN   = VT_IN  - 0.5*FLOWX(I,2,1)*(VT(I,2,1)+VT(I+1,2,1))
      SUM_FLO1 = SUM_FLO1 - 0.5*(FLOWX(I,2,1)+FLOWX(I,2,1))
      SUM_FLOM = SUM_FLOM - 0.5*(FLOWX(I,JMM2,1)+FLOWX(I,JMM2,1))
      AOUT    = AOUT + AQTOT(I,JMM2,1)
      AIN     = AIN  + AQTOT(I,2,1)
      ROIN    = ROIN - 0.5*FLOWX(I,2,1)*(RO(I,2,1)+RO(I+1,2,1))
      ROOUT  = ROOUT - 0.5*FLOWX(I,JMM2,1)*(RO(I,JMM2,1)+RO(I+1,JMM2,1))
      END DO
C
C
C    CALCULATE THE CURRENT FLOW RELATIVE EXIT ANGLE , A2_NOW   .       
C
           VX_IN    = VX_IN/SUM_FLO1
           VX_OUT   = VX_OUT/SUM_FLOM
           VR_IN    = VR_IN/SUM_FLO1
           VR_OUT   = VR_OUT/SUM_FLOM
           VM_IN    = SQRT(VX_IN*VX_IN   + VR_IN*VR_IN)
           VM_OUT   = SQRT(VX_OUT*VX_OUT + VR_OUT*VR_OUT)
           VT_OUT   = VT_OUT/SUM_FLOM
           VT_IN    = VT_IN/SUM_FLO1
           PAVG_OUT = 0.5*(PDOWN_HUB+PDOWN_TIP)
           UBLAD1   = UBLADE(1,1)
           UBLAD2   = UBLADE(JMM2,1)
           WT_IN    = VT_IN  - UBLAD1
           WT_OUT   = VT_OUT - UBLAD2
           WOUT_SQ  = (VM_OUT*VM_OUT + WT_OUT*WT_OUT)
           WREL_OUT = SQRT(WOUT_SQ)
           WT_OUT_COMP = WREL_OUT*SIN(A2_COMP)
           VT_OUT_COMP = WT_OUT_COMP + UBLADE2
           A2_NOW   = ATAN(WT_OUT/VM_OUT) 
           ROIN     = ROIN/SUM_FLO1
           ROOUT    = ROOUT/SUM_FLOM
           RAVG_IN  = 0.5*(R(2,1)  + R(2,2))
           RAVG_OUT = 0.5*(R(JM,1) + R(JM,2))
           PITCH_OUT = 2*PI*RAVG_OUT/NBLADE(JMID)
           SUM_FLO_COMP = VM_OUT*ROOUT*AOUT
C
C     CALCULATE THE CURRENT AND SPECIFIED BLADE TANGENTIAL FORCE.
C
      FBLADE_SPEC = 0.0
      FBLADE_NOW  = 0.0
      DO J=J_LEDGE+1,J_TEDGE
      PDIF_SPEC   = (PSPEC_IM(J)  - PSPEC_I1(J)
     &            +  PSPEC_IM(J-1)- PSPEC_I1(J-1))*0.5
      PDIF_NOW = 0.25*(P(IM,J,1) - P(1,J,1) + P(IM,J-1,1) - P(1,J-1,1)
     &               + P(IM,J,2) - P(1,J,2) + P(IM,J-1,2) - P(1,J-1,2))
      R_AVGG       = 0.25*(R(J,1)+R(J,2)+R(J-1,1)+R(J-1,2))
      FBLADE_SPEC  = FBLADE_SPEC  - PDIF_SPEC*ABT(J,1)*R_AVGG
      FBLADE_NOW   = FBLADE_NOW   - PDIF_NOW*ABT(J,1)*R_AVGG
      END DO
C
      WRITE(6,*)'******************************************************'
      WRITE(6,*)'******************************************************'
...
C************************************************************
C
C    CALCULATE THE COMPATIBLE BLADE EXIT ANGLE - OLD METHOD.
C
      WRITE(6,*) ' OLD METHOD FOR COMPATIBLE ANGLE '
C
      DFORCE       =  FBLADE_SPEC/(ROOUT*RAVG_OUT*AOUT*WOUT_SQ)  
      SINA2_COMP   =  SIN(A2_COMP)
      COSA2_COMP   =  COS(A2_COMP)
C
C
C     ITERATE TO GET FINAL VALUE OF A2_COMP
      SGNOUT  = ABS(A2_SET + 0.001)/A2_SET
      DO NL = 1,5  
      COSA2_COMP1  =  DFORCE/
     & (UBLAD2/WREL_OUT + SINA2_COMP - RAVG_IN*VT_IN/RAVG_OUT/WREL_OUT)
      IF(COSA2_COMP1.GT.0.999)  COSA2_COMP1 = 0.999
      IF(COSA2_COMP1.LT.0.0) COSA2_COMP1 = 0.0
      SINA2_NEW  = SQRT(1.0- COSA2_COMP1*COSA2_COMP1)*SGNOUT 
      SINA2_COMP = 0.9*SINA2_COMP + 0.1*SINA2_NEW
      END DO
C
      WRITE(6,*) ' END OF ITERATION '
C
      COSA2_COMP = COSA2_COMP1
C
      WRITE(6,*)
      WRITE(6,*) ' F_SPEC= ',FBLADE_SPEC,'ROOUT= ',ROOUT,
     &           ' WOUT_SQ= ',WOUT_SQ
      WRITE(6,*) ' DFORCE        = ', DFORCE
      WRITE(6,*) ' WTIN/WREL_OUT = ', WT_IN/WREL_OUT
      WRITE(6,*) ' COS_A2COMP    = ', COSA2_COMP
      WRITE(6,*) ' SIN_A2COMP    = ', SINA2_COMP
      WRITE(6,*) ' A2 IN DEGREES = ', ACOS(COSA2_COMP1)*RADDEG
      WRITE(6,*)
C
      A2_COMP  = 0.5*A2_COMP + 0.5*SGNOUT*ACOS(COSA2_COMP)
C
      JMD = JM/2
      NBLADE_NOW  = NBLADE(JMD)
C   
      NBLADE_COMP = NBLADE_NOW*(FBLADE_NOW/FBLADE_SPEC)
C
      A2_SET   = (1.-ANG_FAK)*A2_SET + ANG_FAK*A2_COMP
C
C************************************************************
C************************************************************
C
      WRITE(6,*)
      WRITE(6,*) ' SPECIFIED BLADE TANGENTIAL FORCE= ', FBLADE_SPEC
      WRITE(6,*) ' CURRENT TANGENTIAL BLADE FORCE  = ', FBLADE_NOW
      WRITE(6,*)
      WRITE(6,*) ' SPECIFIED EXIT FLOW ANGLE       = ', A2_SPEC*RADDEG
      WRITE(6,*) ' CURRENT EXIT FLOW ANGLE         = ', A2_NOW*RADDEG
      WRITE(6,*) ' COMPATIBLE EXIT ANGLE           = ', A2_COMP*RADDEG
      WRITE(6,*) ' CURRENT SETTING EXIT ANGLE TO   = ', A2_SET*RADDEG
      WRITE(6,*)
      WRITE(6,*) ' CURRENT BLADE NUMBER            = ', NBLADE_NOW
      WRITE(6,*) ' COMPATIBLE BLADE NUMBER         = ', NBLADE_COMP
C      
C   SET THE BLADE ROTATION ANGLE
C
      ROTN    = (A2_SET - A2_NOW)*ROT_FAK
C     
      WRITE(6,*)
      WRITE(6,*)'ANGLE OF BLADE ROTATION= ',ROTN*RADDEG,'DEGREES'
      WRITE(6,*)
C
C******************************************************************************
C******************************************************************************
C
C    IF THE CALCULATION HAS FINISHED WRITE OUT THE NEW GRID TO A FILE 'newgrid'
C    ON UNIT 15 AND RETURN FROM THE SURROUTINE.
C
C*******************************************************************************
C*******************************************************************************
      IF(IFEND.EQ.1) THEN
C
      OPEN(UNIT=15,FILE='newgrid')
      WRITE(15,*) '  NEW BLADE  I = 1  SURFACE COORDINATES'
      WRITE(15,*)
      WRITE(15,3999) (RTHETA(1,J,1),J=1,JM)
      WRITE(6,*)
      WRITE(15,*) '  NEW BLADE TANGENTIAL THICKNESS COORDINATES'
      WRITE(15,*)
      WRITE(15,3999) (RT_THICK(J,1),J=1,JM)
 3999 FORMAT(8F10.6)
      CLOSE(15)
C
      RETURN
...
C******************************************************************************
C
C     IF NOT AT THE END OF THE CALCULATION CONTINUE TO UPDATE THE BLADE GEOMETRY
C
C******************************************************************************
C******************************************************************************
C
      WRITE(6,*) ' STARTING TO GENERATE THE NEW BLADE , NSTEPS=', NSTEP
C
C   INITIALISE THE BLADE MOVEMENTS TO ZERO.
C
      DO J=1,JM
      SDIST(J)    = S_MERID(J)
      YMOVE_I1(J) = 0.0
...
C******************************************************************* ***********
C
C    NOW START TO MOVE THE BLADE SURFACES.
C
C******************************************************************************
C******************************************************************************
C
      DUPT  = S_MERID(J_TEDGE)   - S_MERID(J_TEDGE-1)
      DWNT  = S_MERID(J_TEDGE-1) - S_MERID(J_TEDGE-2)
      FEXTE = DUPT/DWNT
C
C    SET THE SOLID BODY ROTATION OF THE WHOLE BLADE
C
      DO 4323 J = J_LEDGE,JM
      DY_ROTN(J)       = ROTN*(S_MERID(J) - S_MERID(J_TEDGE))
 4323 CONTINUE
      DO 4322  J = 1,J_LEDGE-1
      DY_ROTN(J) = DY_ROTN(J_LEDGE)
 4322 CONTINUE
C
C      FIRST MOVE THE I = 1 BLADE SURFACE.
C
      IF(IFINV_I1.GT.0) THEN
C 
      DO 4324 J = J_LEDGE,JM
      YMOVE_I1(J)= SUM_SOURCE_I1(J)/(ROVMER(J)*TK_SURF(J))
 4324 CONTINUE
C
C     EXTRAPOLATE THE TRAILING EDGE POINT
      IF(ISUCT.EQ.1) THEN
      YMOVE_I1(J_TEDGE) = YMOVE_I1(J_TEDGE-1) 
     &  + FEXTE*(YMOVE_I1(J_TEDGE-1) - YMOVE_I1(J_TEDGE-2))
      END IF
C
C     SET THE MOVEMENT UPSTREAM OF THE LEADING EDGE = THAT AT THE LE.
      DO J = 1,J_LEDGE-1
      YMOVE_I1(J) = YMOVE_I1(J_LEDGE)
      END DO
C
      END IF
C  END OF MOVING THE I = 1 BLADE SURFACE
C
C******************************************************************************
C******************************************************************************
C    NOW MOVE THE BLADE I = IM SURFACE
C 
      IF(IFINV_IM.GT.0) THEN
C
C     SET THE MOVEMENT OF THE BLADE THICKNESS I = IM SURFACE.
C     "SUM_SOURCE"  IS +VE FOR OUTWARDS FLOW FROM THE BLADE
C     IN WHICH CASE  "YMOVE"  MUST BE MINUS SUM_SOURCE TO MAKE
C     IT POSITIVE FOR MOVEMENT IN THE POSITIVE THETA DIRECTION.
C
      DO 4325 J = J_LEDGE,JM
      YMOVE_IM(J)   = -SUM_SOURCE_IM(J)/(ROVMER(J)*TK_SURF(J)) 
 4325 CONTINUE
C
C     EXTRAPOLATE THE TRAILING EDGE POINT
C
      IF(ISUCT.EQ.IM) THEN
      YMOVE_IM(J_TEDGE) = YMOVE_IM(J_TEDGE-1)  
     &  + FEXTE*(YMOVE_IM(J_TEDGE-1) - YMOVE_IM(J_TEDGE-2))
      END IF
C
C     SET THE MOVEMENT UPSTREAM OF THE LEADING EDGE = THAT AT THE LE.
      DO J = 1,J_LEDGE-1
      YMOVE_IM(J) = YMOVE_IM(J_LEDGE)
      END DO
C
      END IF
C      END OF MOVING THE I = IM BLADE SURFACE
C******************************************************************************
C******************************************************************************
C
C
C****************************************************************************
C     TRY TO ADJUST THE BLADE THICKNES TO THE SPECIFIED VALUE.
C     IF RF_THICK IS GREATER THAN ZERO.
C****************************************************************************
      IF(RF_THICK.GT.0.001) THEN
C
      RFT  = RF_THICK
      DO J = J_LEDGE,J_TEDGE
           TK_ADD      = TKSET(J)  - RT_THICK(J,1)
           RFNOW       = RFT*(0.5 + 1.0*FRAC_CRD(J))
           IF(ISUCT.EQ.1) THEN
                YMOVE_IM(J) = YMOVE_IM(J) - RFNOW*TK_ADD
           ELSE
                YMOVE_I1(J) = YMOVE_I1(J) + RFNOW*TK_ADD
           END IF
      END DO
C   END OF RF_THICK  > 0  LOOP
      END IF
C
C******************************************************************************
C    END OF ADJUSTING THE BLADE THICKNESS
C    SET THE THICKNESS TO ZERO UPSTREAM OF THE BLADE.
C******************************************************************************
C
      DO J = 1,J_LEDGE
           IF(ISUCT.EQ.1) THEN
      ...
      END DO
C
C     EXTRAPOLATE THE TRAILING EDGE THICKNESS
C
      IF(ISUCT.EQ.1) THEN
      YMOVE_IM(J_TEDGE) = YMOVE_IM(J_TEDGE-1) 
     &  + FEXTE*(YMOVE_IM(J_TEDGE-1) - YMOVE_IM(J_TEDGE-2))
     ...
     &  + FEXTE*(YMOVE_I1(J_TEDGE-1) - YMOVE_I1(J_TEDGE-2))
      END IF
C
C    SET THE THICKNESS TO ZERO DOWNSTREAM OF THE BLADE.
C
      DO J = J_TEDGE+1,JM
           IF(ISUCT.EQ.1) THEN
                YMOVE_IM(J) = 0.0
      ...
      END DO
C
C     SMOOTH THE BLADE MOVEMENT ON THE I = 1 SURFACE.
C
      CALL SMOOTH(J_LEDGE,J_TEDGE,N_SMTH,F_SMTH,S_MERID,YMOVE_I1)
C
C     SMOOTH THE BLADE MOVEMENT ON THE I = IM SURFACE.
C
      CALL SMOOTH(J_LEDGE,J_TEDGE,N_SMTH,F_SMTH,S_MERID,YMOVE_IM)
C
C     END OF SMOOTHING THE BLADE SURFACES
C
C******************************************************************************
C******************************************************************************
C       THE BLADE MOVEMENTS ARE NOW FINALISED.
C       CHANGE THE BLADE SURFACE COORDINATES READY FOR THE NEXT ITERATION.
C**********************************************************************
C*********************************************************************
C
      DO 4326 J= 1,JM
               RTHETA(1,J,1) = RTHETA(1,J,1) + YMOVE_I1(J) + DY_ROTN(J)
               RTHETA(1,J,2) = RTHETA(1,J,1) 
               RT_THICK(J,1) = RT_THICK(J,1) - YMOVE_IM(J) + YMOVE_I1(J)
               IF(RT_THICK(J,1).LT.TKMIN(J)) RT_THICK(J,1) =
     &         0.5*(TKMIN(J) + RT_THICK(J,1))
               RT_THICK(J,2) = RT_THICK(J,1)
 4326 CONTINUE
C
C******************************************************************************
C******************************************************************************
C   EXTRAPOLATE THE BLADE SURFACES TO THE TRAILING EDGE AND
C   SET THE CENTRE LINE DOWNSTREAM OF THE TRAILING EDGE
C******************************************************************************
C******************************************************************************
C
C
      RTHETA(1,J_TEDGE,1) = RTHETA(1,J_TEDGE-1,1) 
     &  + FEXTE*(RTHETA(1,J_TEDGE-1,1) - RTHETA(1,J_TEDGE-2,1)) 
      RTHETA(1,J_TEDGE,2)  =  RTHETA(1,J_TEDGE,1)

      RT_THICK(J_TEDGE,1) = RT_THICK(J_TEDGE-1,1) 
     &  + FEXTE*(RT_THICK(J_TEDGE-1,1) - RT_THICK(J_TEDGE-2,1)) 
      RT_THICK(J_TEDGE,2)  =  RT_THICK(J_TEDGE,1)
C
      TE_NOW   = RT_THICK(J_TEDGE,1)
      YMID_TE  = RTHETA(1,J_TEDGE,1) - 0.5*TE_NOW
      DO J= J_TEDGE+1,JM
      RT_MID(J) = YMID_TE + (S_MERID(J)-S_MERID(J_TEDGE))*TAN(A2_SPEC)
      END DO
C 
C      IF(IFEND.EQ.1.OR.MOD(NSTEP,500).EQ.0) THEN
C      WRITE(6,*) ' BLADE SURFACE MOVEMENTS, JTEDGE =', J_TEDGE
C      DO J = 1,JM
C      WRITE(6,*) 'J,YMOVE_I1,YMOVE_IM,RTHETA,RTTHIK,TKSET', 
C     &            J,YMOVE_I1(J),YMOVE_IM(J),RTHETA(1,J,1),RT_THICK(J,1),
C     &            TKSET(J)
C      END DO
C      END IF
C 
C   FORM A 3 POINT CUSP BEHIND THE TRAILIMG EDGE
C
      RT_THICK(J_TEDGE+1,1) = 0.7*TE_NOW
      RT_THICK(J_TEDGE+2,1) = 0.4*TE_NOW
      RT_THICK(J_TEDGE+3,1) = 0.1*TE_NOW
      RT_THICK(J_TEDGE+1,2) = 0.7*TE_NOW
      RT_THICK(J_TEDGE+2,2) = 0.4*TE_NOW
      RT_THICK(J_TEDGE+3,2) = 0.1*TE_NOW
C
      RTHETA(1,J_TEDGE+1,1) = RT_MID(J_TEDGE+1) + 0.35*TE_NOW
      RTHETA(1,J_TEDGE+2,1) = RT_MID(J_TEDGE+2) + 0.20*TE_NOW
      RTHETA(1,J_TEDGE+3,1) = RT_MID(J_TEDGE+3) + 0.05*TE_NOW
      RTHETA(1,J_TEDGE+1,2) = RT_MID(J_TEDGE+1) + 0.35*TE_NOW
      RTHETA(1,J_TEDGE+2,2) = RT_MID(J_TEDGE+2) + 0.20*TE_NOW
      RTHETA(1,J_TEDGE+3,2) = RT_MID(J_TEDGE+3) + 0.05*TE_NOW
C
      DO J = J_TEDGE+4,JM
      RTHETA(1,J,1) = RT_MID(J)
      RTHETA(1,J,2) = RT_MID(J)
      END DO
C
C******************************************************************************
C                END OF ADJUSTING THE TRAILING EDGE CUSP
C******************************************************************************
C******************************************************************************
C
C    END OF ALL INVERSE MODE OPERATIONS.
C
C*******************************************************************************
C*******************************************************************************
C*******************************************************************************
C
C   THE REST OF THIS SUBROUTINE IS THE SAME AS IN SUBROUTINE SETUP
C   I.E. SETTING UP THE GRID FOR THE NEW GEOMETRY.
C
      WRITE(6,*) ' STORING THE NEW BLADE GEOMETRY, NSTEPS = ', NSTEP
C
CC******************************************************************************
C*******************************************************************************
C
      ...
      DO 3021 K=1,KM
      GAP           = PITCH*R(J,K) - RT_THICK(J,K)
      THETA(1,J,K)  = RTHETA(1,J,K)/R(J,K)
      RT_PITCH(J,K) = PITCH*R(J,K)
      DO 3020 I=2,IM
      RTHETA(I,J,K) = RTHETA(I-1,J,K) + FP(I-1)*GAP
      THETA(I,J,K)  = RTHETA(I,J,K)/R(J,K)
...
C
      ...
      READ(9,*) 
      WRITE(6,*)
      WRITE(6,*) ' READING IN THE DENSITY ALONG THE "I" AXIS '
      WRITE(6,*) ' DENSITY '
      READ(9,*)   (ROAXIS(I),I=1,ITAB)
      WRITE(6,90) (ROAXIS(I),I=1,ITAB)  
   90 FORMAT(8F15.6)
C
      DO I=1,ITAB-1
      DROAXIS(I) = ROAXIS(I+1) - ROAXIS(I)
      END DO
C
      WRITE(6,*)
      WRITE(6,*) ' READING IN THE INTERNAL ENERGY ALONG THE "J" AXIS'
      READ(9,*) 
      WRITE(6,*) ' INTERNAL ENERGY '
      READ(9,*)  (UAXIS(J),J=1,JTAB)
      WRITE(6,91)(UAXIS(J),J=1,JTAB)
   91 FORMAT(8F15.3)
      DO J =1,JTAB-1
      DUAXIS(J) = UAXIS(J+1) - UAXIS(J)
      END DO
C  
C    READ IN DENSITY
C
      ...
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED DENSITY'
      DO 110 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      READ(9,93) (RO_TAB(I,J),J=1,JTAB)
  110 CONTINUE
   93 FORMAT(8E15.8)
C  
C    READ IN INTERNAL ENERGY
C
      WRITE(6,*) 
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED INTERNAL ENERGY'
      DO 120 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      READ(9,93) (U_TAB(I,J),J=1,JTAB)
  120 CONTINUE
C  
C    READ IN PRESSURE
C
      ...
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED PRESSURE'
      DO 130 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      READ(9,93) (P_TAB(I,J),J=1,JTAB)
  130 CONTINUE
C
...
C
      ...
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED TEMPERATURE '
      DO 150 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      READ(9,93) (T_TAB(I,J),J=1,JTAB)
  150 CONTINUE
C
C    READ IN ENTROPY
C
      ...
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED ENTROPY '
      DO 160 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      READ(9,93) (ENT_TAB(I,J),J=1,JTAB)
  160 CONTINUE
C
C    READ IN P-H EXPONENT
C
      WRITE(6,*)
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED ISENTROPIC INDEX, GA_PV '
      DO 190 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      READ(9,93) (GA_PV_TAB(I,J),J=1,JTAB)
  190 CONTINUE
C
C    READ IN DRYNESS
C
      ...
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
      WRITE(6,*) ' READING IN THE TABULATED DRYNESS '
      DO 180 I = 1 , ITAB
      READ(9,*)  DUMMY
      READ(9,*)  DUMMY
...
C
      WRITE(6,*)
      WRITE(6,*) ' CHECKING THE PRESSURE '
      CALL CHECK_TAB(ITB,JTB,ITAB,JTAB,P_TAB)
      WRITE(6,*)
      WRITE(6,*) ' CHECKING TEMPERATURE'
      CALL CHECK_TAB(ITB,JTB,ITAB,JTAB,T_TAB)
      WRITE(6,*)
      WRITE(6,*) ' CHECKING ENTROPY '
      CALL CHECK_TAB(ITB,JTB,ITAB,JTAB,ENT_TAB)
      WRITE(6,*)
      WRITE(6,*) ' CHECKING GAMMA'
      CALL CHECK_TAB(ITB,JTB,ITAB,JTAB,GA_PV_TAB)
      WRITE(6,*)
...
C
      NXTRAP = 2
      UMINN  = 2*UAXIS(1)     - UAXIS(1+NXTRAP)
      UMAXX  = 2*UAXIS(JTAB)  - UAXIS(JTAB-NXTRAP)
      ROMINN = 2*ROAXIS(1)    - ROAXIS(1+NXTRAP)
      ROMAXX = 2*ROAXIS(ITAB) - ROAXIS(ITAB-NXTRAP)
C
      PMAX = 0.0
      PMIN = 1.0E10  
      TMAX = 0.0
      ...
      DO 95 I = 1,ITAB
      DO 95 J = 1,JTAB
      IF(RO_TAB(I,J).LT.ROMIN) ROMIN = RO_TAB(I,J)
      IF(RO_TAB(I,J).GT.ROMAX) ROMAX = RO_TAB(I,J)
      IF(U_TAB(I,J).LT.UMIN) UMIN = U_TAB(I,J)
      IF(U_TAB(I,J).GT.UMAX) UMAX = U_TAB(I,J)
      IF(P_TAB(I,J).LT.PMIN) PMIN = P_TAB(I,J)
      IF(P_TAB(I,J).GT.PMAX) PMAX = P_TAB(I,J)
      IF(T_TAB(I,J).LT.TMIN) TMIN = T_TAB(I,J)
      IF(T_TAB(I,J).GT.TMAX) TMAX = T_TAB(I,J)
      IF(ENT_TAB(I,J).GT.ENTMAX)  ENTMAX = ENT_TAB(I,J)
      IF(ENT_TAB(I,J).LT.ENTMIN)  ENTMIN = ENT_TAB(I,J)
      IF(DRY_TAB(I,J).GT.DRYMAX)  DRYMAX = DRY_TAB(I,J)
      IF(DRY_TAB(I,J).LT.DRYMIN)  DRYMIN = DRY_TAB(I,J)
      IF(GA_PV_TAB(I,J).GT.GA_PV_MAX)  GA_PV_MAX = GA_PV_TAB(I,J)
      IF(GA_PV_TAB(I,J).LT.GA_PV_MIN)  GA_PV_MIN = GA_PV_TAB(I,J)
   95 CONTINUE
C
      WRITE(6,*)'******************************************************'
      WRITE(6,*)
      WRITE(6,*) ' FINISHED READING THE LOOK UP TABLE '
      WRITE(6,*) ' MAXIMUM AND MINIMUM PROPERTIES TABULATED ARE AS FOLLO
     &WS'
      WRITE(6,*)
C
      WRITE(6,*) ' ITAB ,  JTAB   = ', ITAB,JTAB
      WRITE(6,*) ' PMAX, PMIN     = ', PMAX,PMIN
      WRITE(6,*) ' TMAX, TMIN     = ', TMAX,TMIN
      WRITE(6,*) ' RHOMAX, RHOMIN = ', ROMAX,ROMIN
      WRITE(6,*) ' UMAX  , UMIN   = ', UMAX, UMIN
      WRITE(6,*) ' ENTMAX, ENTMIN = ', ENTMAX,ENTMIN
      WRITE(6,*) ' GA_PV_MAX, GA_PV_MIN = ', GA_PV_MAX,GA_PV_MIN
      WRITE(6,*) ' DRYMAX, DRYMIN = ', DRYMAX, DRYMIN
      WRITE(6,*) 
      WRITE(6,*) 'FINISHED READING AND CHECKING THE GAS PROPERTY TABLES'
      WRITE(6,*)
      WRITE(6,*)'******************************************************'
C
      CLOSE(9)
...
C******************************************************************************
C
      SUBROUTINE CHECK_TAB(ITB,JTB,ITAB,JTAB,VAR)
C
      DIMENSION VAR(ITB,JTB)
C
C      CHECK FOR ANY LARGE VARIATIONS BETWEEN ADJACENT POINTS
C
      DO 94  I = 1,ITAB
      DO 94, J = 2,JTAB-1
      VARM1 = VAR(I,J-1)
      VARJ  = VAR(I,J)
      VARP1 = VAR(I,J+1)
      IF(ABS(VARJ - VARM1)/ABS(VARM1).GT.0.1)THEN
      VAR(I,J) = 0.5*(VARM1 + VARP1)
      WRITE(6,*) ' CHANGED VALUE AT I= , J= ',I,J,
     & ' OLD VALUES AT J-1,J,J+1 = ',VARM1,VARJ,VARP1,
     & ' NEW VALUE AT J = ',VAR(I,J)
      END IF
   94 CONTINUE
C
      RETURN
...
C     THREAD-LOCAL HINT: previous search result for fast path
      ...
      COMMON /TABHINT/ IHINT, JHINT, IHINT_OK
C$OMP THREADPRIVATE(/TABHINT/)
      DATA IHINT /1/, JHINT /1/, IHINT_OK /0/
C
C******************************************************************************
C******************************************************************************
C   CHECK THAT THE DENSITY AND ENERGY VALUES ARE WITHIN THE TABULATED RANGE.
C
       IF(RHOIN.LT.ROMINN) RHOIN = ROMINN
       IF(RHOIN.GT.ROMAXX) RHOIN = ROMAXX
       IF(UIN.LT.UMINN)      UIN = UMINN
       IF(UIN.GT.UMAXX)      UIN = UMAXX
C
C     TRY HINT FROM PREVIOUS CALL (FAST PATH)
      IF(IHINT_OK.EQ.1) THEN
        IF(ROAXIS(IHINT).LE.RHOIN .AND.
     &     RHOIN.LT.ROAXIS(IHINT+1)) THEN
          IFOUND = IHINT
        ELSE
          GO TO 100
        END IF
        IF(UAXIS(JHINT).LE.UIN .AND.
     &     UIN.LT.UAXIS(JHINT+1)) THEN
          JFOUND = JHINT
        ELSE
          GO TO 200
        END IF
      ...
      END IF
C
C     FULL SEARCH FOR DENSITY "I" AXIS
 100  CONTINUE
             ISTART = 1
             IEND   = I_MID-1
 ...
 211  CONTINUE
C
C     FULL SEARCH FOR INTERNAL ENERGY "J" AXIS
 200  CONTINUE
             JSTRT = 1
             JEND   = J_MID-1
...
C******************************************************************************
C
      DIFRO = (RHOIN - ROAXIS(IFOUND))/DROAXIS(IFOUND)
      DIFU  = (UIN   -  UAXIS(JFOUND))/DUAXIS(JFOUND)
C
C******************************************************************************
C
...
C***********************************************************************************
C
      SUBROUTINE FINDIT(ITB,JTB,IFOUND,JFOUND,IFP1,JFP1,
...
C
     ...
     &     + VAR(IFP1,JFP1)    - VAR(IFP1,JFOUND)
C
      VCORNER   = VAR(IFOUND,JFOUND)
      ROCORR_N  = DIFRO*DFDI
      UCORR_N   = DIFU*DFDJ
      VFOUND    = VCORNER + 0.5*(ROCORR_N + UCORR_N) 
C
      RETURN
      END
